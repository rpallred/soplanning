...
<form class="form-horizontal" id="formIcal">
	...
	<div class="form-group row">
		...
		<div class="col-md-8 radio-inline">
			...
			<div id="divIcalProjets" class="hidden">
					<select multiple="multiple" name="icalProjetsChoix" id="icalProjetsChoix" class="form-control {if !$smarty.session.isMobileOrTablet}select2{/if}" onfocus="blur();" tabindex="1" style="width:100%" onchange="xajax_icalGenererLien($('#anciennete').val(), getRadioValue('ical_users'), getRadioValue('ical_projets'), getSelectValue('icalProjetsChoix'), $('#anciennete').val());">
					<option value="">- - - - - - - - - - -</option>
					...
					{foreach from=$listeProjets item=projet}
						...
						{/if}
						<option value="{$projet.projet_id}">{$projet.nom|xss_protect} ({$projet.projet_id}) {if $projet.livraison neq ''} - S{$projet.livraison}{/if}</option>
					{/foreach}
				</select>
			</div>
			<br />
	...
	<div class="form-group row">
		<label class="col-md-3 col-form-label">{#icalExport_download#} :</label>
		<div class="col-md-6 form-inline">
			<a id="downloadIcal" href="export_ical"><img src="assets/img/pictos/download.png" /></a>
