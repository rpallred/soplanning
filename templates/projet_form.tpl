<form method="POST" action="" target="_blank" id="projectForm">
		...
		<div class="col-md-6">
			<select name="groupe_id" id="groupe_id" class="form-control select2">
				...
				{foreach from=$groupes item=groupe}
					<option value="{$groupe.groupe_id}" {if $projet.groupe_id eq $groupe.groupe_id}selected="selected"{/if}>{$groupe.nom|xss_protect}</option>
				{/foreach}
			</select>
		</div>
	</div>
		...
		<div class="col-md-6">
			<select class="form-control" name="statut" id="statut">
				{foreach from=$listeStatus item=status}
					<option value="{$status.status_id}" {if (isset($projet.statut) and $projet.statut eq $status.status_id) or (!isset($projet.statut) and $defaut_status eq $status.status_id)}selected="selected"{/if}>{$status.nom|xss_protect}</option>
				{/foreach}
			</select>
		</div>
	</div>
	...
	<div class="form-group row col-md-12">
		...
		<div class="col-md-6">
			<input class="form-control" name="lien" id="lien" {if $smarty.session.isMobileOrTablet==1}type="url"{else}type="text"{/if} maxlength="255" value="{$projet.lien|xss_protect}" />
		</div>
		{if $projet.lien neq ""}
			<div class="col-md-2">
				<a class="btn btn-default tooltipster" title="{#winProjet_gotoLien#|escape}" href="{if $projet.lien|strpos:"http" !== 0 && $projet.lien|strpos:"\\" !== 0}http://{/if}{$projet.lien|xss_protect}" target="_blank"><i class="fa fa-share-square-o" aria-hidden="true"></i></a>
			</div>
		{/if}
	</div>
	<div class="form-group row col-md-12">
