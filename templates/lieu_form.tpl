...
<form method="POST" action="" target="_blank" class="form-horizontal">
	...
	<div class="form-group row col-md-12 align-items-center">
		...
		<div class="col-md-3">
			<input name="couleur" id="couleur" maxlength="6" {if $smarty.session.isMobileOrTablet==1}type="color"{else}type="text"{/if} value="#{if $lieu.couleur eq ''}ffffff{else}{$lieu.couleur}{/if}" />
		</div>
	</div>
	<div class="form-group row col-md-12 align-items-center">
	...
	<div class="form-group row col-md-12 align-items-center">
		...
		<div class="col-md-5">
			<br />
			<input type="button" value="{#enregistrer#|escape:"html"}" class="btn btn-primary" onClick="xajax_submitFormLieu('{$lieu.lieu_id}', $('#lieu_id').val(), $('#nom').val(), $('#commentaire').val(), $('#lieu_exclusif').is(':checked'), $('#couleur').val())" />
		</div>
	</div>
</form>
<script>
