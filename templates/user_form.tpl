	...
	<div class="tab-content">	
		<div class="tab-pane active" id="droits">
			...
			<div class="row">
				...
				<div class="col-form-label" style="width:70%">					
					<div class="form-check form-check-inline">
						<input class="form-check-input" type="radio" name="planning_view" id="tasks_view_all_projects" value="tasks_view_all_projects" {if in_array("tasks_view_all_projects", $user_form.tabDroits) || !in_array("tasks_view_own_projects", $user_form.tabDroits)}checked="checked"{/if} onChange="{literal}if(this.checked){document.getElementById('divSpecificProjets').style.display='none';}{/literal}">
						<label class="form-check-label" for="tasks_view_all_projects">{#droits_vueTousProjets#}</label>
					</div>
					<div class="form-check form-check-inline">
						<input class="form-check-input" type="radio" name="planning_view" id="tasks_view_team_projects" value="tasks_view_team_projects" {if in_array("tasks_view_team_projects", $user_form.tabDroits)}checked="checked"{/if} onChange="{literal}if(this.checked){document.getElementById('divSpecificProjets').style.display='none';}{/literal}">
						<label class="form-check-label" for="tasks_view_team_projects">{#droits_vueProjetsEquipe#}</label>
					</div>
					<div class="form-check form-check-inline">
						<input class="form-check-input" type="radio" name="planning_view" id="tasks_view_own_projects" value="tasks_view_own_projects" {if in_array("tasks_view_own_projects", $user_form.tabDroits)}checked="checked"{/if} onChange="{literal}if(this.checked){document.getElementById('divSpecificProjets').style.display='none';}{/literal}">
						<label class="form-check-label" for="tasks_view_own_projects">{#droits_vueProjetsAssignes#}</label>
					</div>						
					<div class="form-check form-check-inline">
						<input class="form-check-input" type="radio" name="planning_view" id="tasks_view_only_own" value="tasks_view_only_own" {if in_array("tasks_view_only_own", $user_form.tabDroits)}checked="checked"{/if} onChange="{literal}if(this.checked){document.getElementById('divSpecificProjets').style.display='none';}{/literal}">
						<label class="form-check-label" for="tasks_view_only_own">{#droits_tasks_view_only_own#}</label>
					</div>
					<div class="form-check form-check-inline">
						<input class="form-check-input" type="radio" name="planning_view" id="tasks_view_specific_projects" value="tasks_view_specific_projects" {if in_array("tasks_view_specific_projects", $user_form.tabDroits)}checked="checked"{/if} onChange="{literal}if(this.checked){document.getElementById('divSpecificProjets').style.display='inline-block';}{/literal}">
						<label class="form-check-label" for="tasks_view_specific_projects">{#droits_tasks_view_specific_projects#}</label>
					</div>
					<div id="divSpecificProjets" style="display:{if in_array("tasks_view_specific_projects", $user_form.tabDroits)}inline-block{else}none{/if}" class="col-form-label">
						<select name="specific_projet_id" multiple="multiple" id="specific_projet_id" class="d-none multiselect">
							...
							{else}
								...
								{foreach from=$listeProjets item=projetCourant name=loopProjets}
									{if $projetCourant.groupe_id neq $groupeTemp}
										</optgroup><optgroup id="g{$projetCourant.groupe_id}" value="1" label="{$projetCourant.groupe_nom|xss_protect}">
									{/if}
								<option value="{$projetCourant.projet_id}" {if in_array($projetCourant.projet_id, $listProjetsRights)}selected="selected"{/if}>{$projetCourant.nom|xss_protect} ({$projetCourant.projet_id|xss_protect})</option> 								
						...
						{literal}
						<script>
							$("#specific_projet_id").multiselect({
								selectAll:true,
								selectGroup:true,
								...
								placeholder: '{/literal}{#formChoixProjet#}{literal}',
								texts: {
						...
						{else}
							...
							{foreach from=$listeUsers item=userCourant name=loopUsers}
								{if $userCourant.user_groupe_id neq $groupeTemp}
									</optgroup><optgroup id="g{$userCourant.user_groupe_id}" value="1" label="{$userCourant.groupe_nom|xss_protect}">
								{/if}
		...
		<div class="tab-pane fade" id="perso">
				...
				<div>
					...
					{if $smarty.const.CONFIG_PROJECT_COLORS_POSSIBLE neq ""}
						{if $smarty.session.isMobileOrTablet==1}
							...
							<datalist id="colors">
								{foreach from=$smarty.const.CONFIG_PROJECT_COLORS_POSSIBLE|split:"," item=couleurTmp}
			...
			<div class="form-group row">
				<label class="col-form-label col-md-3">{#langue#} :</label>
				<div class="form-check form-check-inline">
					<select id="langue" class="form-control">
						<option value="">- - - - -</option>
						<option value="fr" {if $user_form.langue eq "fr"}selected="selected"{/if}>French</option>
						<option value="en" {if $user_form.langue eq "en"}selected="selected"{/if}>English</option>
						<option value="nl" {if $user_form.langue eq "nl"}selected="selected"{/if}>Dutch</option>
						<option value="it" {if $user_form.langue eq "it"}selected="selected"{/if}>Italian</option>
						<option value="es" {if $user_form.langue eq "es"}selected="selected"{/if}>Spanish</option>
						<option value="de" {if $user_form.langue eq "de"}selected="selected"{/if}>German</option>
						<option value="pt" {if $user_form.langue eq "pt"}selected="selected"{/if}>Portuguese</option>
						<option value="br" {if $user_form.langue eq "br"}selected="selected"{/if}>Brazilian Portuguese</option>
						<option value="pl" {if $user_form.langue eq "pl"}selected="selected"{/if}>Polish</option>
						<option value="da" {if $user_form.langue eq "da"}selected="selected"{/if}>Danish</option>
						<option value="hu" {if $user_form.langue eq "hu"}selected="selected"{/if}>Hungarian</option>
						<option value="id" {if $user_form.langue eq "id"}selected="selected"{/if}>Indonesian</option>
						<option value="sl" {if $user_form.langue eq "sl"}selected="selected"{/if}>Slovenian</option>
					</select>
				</div>
