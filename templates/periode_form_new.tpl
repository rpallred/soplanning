...
<form class="form-horizontal" method="POST" target="_blank" id="periodForm">
	...
	<div class="form-group row col-md-12">
		...
		<div class="col-md-10">
			<input type="text" placeholder="{#js_choisirTitre#}" class="form-control" name="titre" id="titre" maxlength="2000" value="{$periode.titre|xss_protect}" onFocus="xajax_autocompleteTitreTache($('#projet2_id').val());"   data-provide="typeahead" tabindex="21" />
		</div>
	</div>
	<div class="form-group row col-md-12">
			<label class="col-md-2 col-form-label">{#winPeriode_projet#}</label>
			<div class="col-md-10">
				<select name="projet2_id" data-placeholder="{#js_choisirProjet#}" id="projet2_id" class="form-control {if !$smarty.session.isMobileOrTablet}select2{/if}" tabindex="1" style="width:100%">
					<option value=""></option>
	...
	<div class="form-group row col-md-12">
				...
				<option value=""></option>
					...
					{foreach from=$listeUsers item=userCourant name=loopUsers}
						{if $userCourant.user_groupe_id neq $groupeTemp}
							<optgroup label="{$userCourant.groupe_nom|xss_protect}">
						{/if}
						...
						>{$userCourant.nom|xss_protect}</option>
						{if $userCourant.user_groupe_id neq $groupeTemp}
							</optgroup>
						{/if}
		...
		<div class="form-group row col-md-12">
			...
			<label class="col-form-label" id="divFinChoixDateLabel">{#winPeriode_fin#}</label>
			...
			<label class="col-form-label" id="divNbJoursLabel">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{#winPeriode_ouNBJours#}&nbsp;&nbsp;</label>
			
			<div style="width:40px;" id="divNbJours">
			<input type="text" class="form-control" name="nb_jours" id="nb_jours"  onChange="videChampsFinTache(this.id);" tabindex="10" />
			</div>
		</div>
		<div class="form-group row col-md-12">
			...
			<div class="btn-group btn-group-toggle" data-toggle="buttons">
				<label class="btn btn-sm btn-outline-light{if $periode.duree_details eq ''} active{/if}" style="border-color:#ced4da;color:#495057" onclick="videChampsFinTache(this.id);$('#divFinChoixDate').removeClass('d-none');$('#divFinChoixDateLabel').removeClass('d-none');$('#divNbJoursLabel').removeClass('d-none');$('#divNbJours').removeClass('d-none');$('#divFinChoixDuree').addClass('d-none');$('#divFinChoixDureeLabel').addClass('d-none');">
					<input type="radio" name="options" id="option1" autocomplete="off" {if $periode.duree_details eq ''}checked{/if}> {#horaire_journee#}
				</label>
				<label class="btn btn-sm btn-outline-light" style="border-color:#ced4da;color:#495057" onclick="videChampsFinTache(this.id);$('#divFinChoixDate').addClass('d-none');$('#divFinChoixDateLabel').addClass('d-none');$('#divNbJoursLabel').addClass('d-none');$('#divNbJours').addClass('d-none');$('#divFinChoixDuree').addClass('d-none');">
				...
				<label class="btn btn-sm btn-outline-light{if $periode.duree_details neq ''} active{/if}" style="border-color:#ced4da;color:#495057" onclick="videChampsFinTache(this.id);$('#divFinChoixDate').addClass('d-none');$('#divFinChoixDateLabel').addClass('d-none');$('#divNbJoursLabel').addClass('d-none');$('#divNbJours').addClass('d-none');$('#divFinChoixDuree').removeClass('d-none');">
					<input type="radio" name="options" id="option3" {if $periode.duree_details neq ''}checked{/if}> {#horaire_custom#}
				</label>
			</div>
			</div>
		</div>
			<div class="row col-md-12 form-inline {if $periode.duree_details eq ''}d-none{/if}" id="divFinChoixDuree">
				...
				<div class="offset-md-2 col-md-10">
					...
					<br>{#ou_horaire#}{#date_de#} <input type="time" class="form-control" autocomplete="off" id="heure_debut" id="heure_debut" size="3"  value="{if isset($periode.duree_details_heure_debut)}{$periode.duree_details_heure_debut|sqltime2usertime}{/if}" onChange="heurefinSynchro(this.value,{$smarty.const.CONFIG_PLANNING_DUREE_CRENEAU_HORAIRE});" tabindex="13" />
					{#date_a#} 
		...
		<label class="col-md-3 col-form-label">{#winPeriode_statut#}</label>
			<div class="col-md-3">
				...
				{foreach from=$listeStatus item=status}
					<option value="{$status.status_id}" {if (isset($periode.statut_tache) and $periode.statut_tache eq $status.status_id) or (!isset($periode.statut_tache) and $defaut_status eq $status.status_id)}selected="selected"{/if}>{$status.nom|xss_protect}</option>
				{/foreach}
				</select>
			</div>
			...
			<div class="col-md-3" >
				<select name="livrable" id="livrable" class="form-control" tabindex="20">
					...
					<div class="form-group row">
						...
						<div class="col-md-8">
							<textarea class="form-control" rows="1" id="notes" name="notes" tabindex="23" style="min-height:50px;max-height:200px">{$periode.notes_xajax|xss_protect}</textarea>
						</div>
					</div>
				</div>
