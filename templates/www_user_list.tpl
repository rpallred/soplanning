	...
	<div class="row">
		<div class="col-md-12">
			<div class="soplanning-box form-inline">
				<div class="btn-group">
					<a href="javascript:xajax_modifUser('');void(0);" class="btn btn-default" ><i class="fa fa-user-plus fa-lg fa-fw" aria-hidden="true"></i>&nbsp;&nbsp;{#menuCreerUser#}</a>
					<a href="{$BASE}/user_groupes" class="btn btn-default"><i class="fa fa-users fa-lg fa-fw" aria-hidden="true"></i>&nbsp;&nbsp;{#menuGroupesUsers#}</a>
				</div>
				...
				<div class="btn-group ml-md-1">
					<a href="javascript:xajax_usersBulkRightsForm();void(0);" class="btn btn-default" ><i class="fa fa-files-o fa-lg fa-fw" aria-hidden="true"></i>&nbsp;&nbsp;{#menuUsersBulkRights#}</a>
				</div>
			</div>
		</div>
	</div>
					...
					<table class="table table-striped table-hover" id="userTab">
						<tr>
							...
							<th class="userTabColId">
								{if $order eq "user_id"}
									{if $by eq "asc"}
										<a href="{$BASE}/user_list?page=1&order=user_id&by=desc">{#user_liste_identifiant#}</a>&nbsp;<img src="{$BASE}/assets/img/pictos/desc_order.png" alt="" />
									{else}
										<a href="{$BASE}/user_list?page=1&order=user_id&by=asc">{#user_liste_identifiant#}</a>&nbsp;<img src="{$BASE}/assets/img/pictos/asc_order.png" alt="" />
									{/if}
								{else}
									<a href="{$BASE}/user_list?page=1&order=user_id&by={$by}">{#user_liste_identifiant#}</a>
								{/if}
							</th>
							<th>
								{if $order eq "nom"}
									{if $by eq "asc"}
										<a href="{$BASE}/user_list?page=1&order=nom&by=desc">{#user_liste_nom#} ({$nbTotalUsers})</a>&nbsp;<img src="{$BASE}/assets/img/pictos/desc_order.png" alt="" />
									{else}
										<a href="{$BASE}/user_list?page=1&order=nom&by=asc">{#user_liste_nom#} ({$nbTotalUsers})</a>&nbsp;<img src="{$BASE}/assets/img/pictos/asc_order.png" alt="" />
									{/if}
								{else}
									<a href="{$BASE}/user_list?page=1&order=nom&by={$by}">{#user_liste_nom#} ({$nbTotalUsers})</a>
								{/if}
							</th>
							<th class="userTabColEmail">
								{if $order eq "email"}
									{if $by eq "asc"}
										<a href="{$BASE}/user_list?page=1&order=email&by=desc">{#user_liste_email#}</a>&nbsp;<img src="{$BASE}/assets/img/pictos/desc_order.png" alt="" />
									{else}
										<a href="{$BASE}/user_list?page=1&order=email&by=asc">{#user_liste_email#}</a>&nbsp;<img src="{$BASE}/assets/img/pictos/asc_order.png" alt="" />
									{/if}
								{else}
									<a href="{$BASE}/user_list?page=1&order=email&by={$by}">{#user_liste_email#}</a>
								{/if}
							</th>
							...
							<th class="wrap d-none d-lg-table-cell">
								{if $order eq "visible_planning"}
									{if $by eq "asc"}
										<a href="{$BASE}/user_list?page=1&order=visible_planning&by=desc">{#user_visiblePlanning#}</a>&nbsp;<img src="{$BASE}/assets/img/pictos/desc_order.png" alt="" />
									{else}
										<a href="{$BASE}/user_list?page=1&order=visible_planning&by=asc">{#user_visiblePlanning#}</a>&nbsp;<img src="{$BASE}/assets/img/pictos/asc_order.png" alt="" />
									{/if}
								{else}
									<a href="{$BASE}/user_list?page=1&order=visible_planning&by={$by}">{#user_visiblePlanning#}</a>
								{/if}
							</th>
						...
						{foreach name=users item=userTmp from=$users}
							<tr>
								<td class="w140 nowrap">
									<a href="javascript:xajax_modifUser('{$userTmp.user_id|escape}');void(0);"><i class="fa fa-pencil fa-lg fa-fw" aria-hidden="true"></i></a>
									<a href="javascript:xajax_supprimerUser('{$userTmp.user_id|escape}');void(0);" onClick="javascript:return confirm('{#user_liste_confirmSuppr#|escape:"javascript"}')"><i class="fa fa-trash-o fa-lg fa-fw" aria-hidden="true"></i></a>
									<a class="tooltipster" href="{$BASE}/process/planning?filtreSurUser={$userTmp.user_id}" title="{#planning_filtre_sur_user#|escape}"><i class="fa fa-calendar fa-lg fa-fw" aria-hidden="true"></i></a>
									<a class="tooltipster" href="javascript:xajax_modifUser('', '{$userTmp.user_id|escape}');void(0);" title="{#user_copier_droits#}"><i class="fa fa-clone fa-lg fa-fw" aria-hidden="true"></i></a>
								</td>
								...
								<td class="wrap">{$userTmp.nom|xss_protect}</td>
						...
						{if $nbPages > 1}
							<tr>
								<td colspan="7" class="text-right" style="white-space:normal">
									{if $currentPage > 1}<a href="{$BASE}/user_list?page={$currentPage-1}">&lt;&lt; {#action_precedent#}</a>&nbsp;&nbsp;{/if}
									{section name=pagination loop=$nbPages}
										{if $smarty.section.pagination.iteration == $currentPage}<b>{else}<a href="{$BASE}/user_list?page={$smarty.section.pagination.iteration}">{/if}
										{$smarty.section.pagination.iteration}
										{if $smarty.section.pagination.iteration == $currentPage}</b>{else}</a>{/if}&nbsp;
									{/section}
									{if $currentPage < $nbPages}<a href="{$BASE}/user_list?page={$currentPage+1}">{#action_suivant#} &gt;&gt;</a>{/if}
								</td>
							</tr>
						{/if}
					</table>
