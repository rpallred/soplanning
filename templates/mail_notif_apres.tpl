...
{assign var=titre_email value="mail_notif_apres"}

{include file="mail_header.tpl"}

{#mail_notif_apres_corps#}
...
{if isset($PersonnesAssigneesListe) && $PersonnesAssigneesListe|@count > 0}
	<br>
	<b>{#personnes_assignees#}</b> : 
	{foreach from=$PersonnesAssigneesListe item=personne name=loopPersonnes}
		{$personne.nom}
		{if !$smarty.foreach.loopPersonnes.last}-{/if}
	{/foreach}
	<br>
{/if}
<br/>
...
{if $periode.date_fin neq ""}
	<b>{#winPeriode_fin#}</b> : {$periode.date_fin|sqldate2userdate}
	<br>
{else}
	<b>{#mail_tacheDuree#}</b> : {$periode.duree|sqltime2usertime}
	<br/>
{/if} 
