		...
		<div class="col-md-8" style="padding-top:7px">
			...
			{foreach from=$tarifs item=tarif}
				<tr>
					...
					<td>
						&nbsp;
						<a href="javascript:xajax_projet_couts_supprimer_personne('{$projet.projet_id}', '{$tarif.projet_user_tarif_id}');void(0);" onclick="javascript: return confirm('{#winPeriode_supprimer#|xss_protect}')"><i style="margin-top:9px" class="fa fa-trash-o fa-lg fa-fw" aria-hidden="true"></i></a>
					</td>
				</tr>
			{/foreach}
			</table>
		...
		<div class="col-md-8">
			<select multiple="multiple" id="user_id_couts" class="form-control {if $smarty.session.isMobileOrTablet!=1}select2{/if}" style="width:70%;display:inline-block">
				...
				{foreach from=$listeUsers item=userCourant name=loopUsers}
					...
					{/if}
					<option value="{$userCourant.user_id}" >{$userCourant.nom|xss_protect}</option>
					{if $userCourant.user_groupe_id neq $groupeTemp}
						</optgroup>
					{/if}
