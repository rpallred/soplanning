...
<form method="POST" action="" target="_blank">
	...
	<div class="form-group row col-md-12 align-items-center">
		<label class="col-md-5 col-form-label">{#status_inclure_dans_calcul#} :</label>
		<div class="col-md-3">
			<select name='inclure_dans_calcul' id='inclure_dans_calcul' class="form-control">
				<option value="oui" {if $status.inclure_dans_calcul == "oui"}selected="selected"{/if}>{#oui#}</option>
				<option value="non" {if $status.inclure_dans_calcul == "non"}selected="selected"{/if}>{#non#}</option>
			</select>
		</div>	
			<div title="{#options_aide_priorite_statut#}" class="cursor-help tooltipster"><i class="fa fa-question-circle" aria-hidden="true"></i></div>
	</div>
	<div class="form-group row col-md-12 align-items-center">
		...
		<div class="col-md-5">
			<br />
			<input type="button" value="{#enregistrer#|escape:"html"}" class="btn btn-primary" onClick="xajax_submitFormStatus('{$status.status_id}', $('#status_id').val(), $('#nom').val(), $('#commentaire').val(), $('#affichage option:selected').val(), $('#button_strikethrough').hasClass('btn-info'),$('#button_bold').hasClass('btn-info'),$('#button_italic').hasClass('btn-info'),$('#button_underline').hasClass('btn-info'), $('#defaut option:selected').val(), $('#affichage_liste option:selected').val(), $('#pourcentage option:selected').val(), {if $smarty.const.CONFIG_PROJECT_COLORS_POSSIBLE neq ""}$('#couleur2 option:selected').val(){else}$('#couleur').val(){/if}, $('#priorite option:selected').val(), $('#inclure_dans_calcul option:selected').val())" />
		</div>
	</div>
</form>
<script>
