<div class="vw-100 position-fixed" id="firstLayer">
	<div class="soplanning-box form-inline pt-0" id="divPlanningDateSelector">
		<div class="btn-group cursor-pointer pt-2" id="btnDateNow">
			<a class="btn btn-default tooltipster" title="{#aujourdhui#}{$dateToday}" onClick="document.location='process/planning?raccourci_date=aujourdhui'" id="buttonDateNowSelector"><i class="fa fa-home fa-lg fa-fw" aria-hidden="true"></i></a>
		</div>
		...
		<div class="btn-group ml-md-2 pt-2 cursor-pointer" id="btnDateSelector">
			<a class="btn btn-default" onClick="document.location='process/planning?raccourci_date=-{$nbJours}';" id="buttonDatePrevSelector"><i class="fa fa-angles-left fa-lg fa-fw" aria-hidden="true"></i><span class="d-none d-xl-inline-block">{$dateBoutonInferieur}</span></a>
			<a class="btn btn-default" onClick="document.location='process/planning?raccourci_date=+{$nbJours}';" id="buttonDateNextSelector"><span class="d-none d-xl-inline-block">{$dateBoutonSuperieur}</span> <i class="fa fa-angles-right fa-lg fa-fw" aria-hidden="true"></i></a>
		</div>
...
<div class="vw-100 position-fixed" id="secondLayer">
	<div class="soplanning-box form-inline pt-0" id="divPlanningMainFilter">
				...
				{else}
					...
					{foreach from=$listeUsers item=userCourant name=loopUsers}
						{if $userCourant.user_groupe_id neq $groupeTemp}
							</optgroup><optgroup id="gu{$userCourant.user_groupe_id}" label="{$userCourant.groupe_nom|xss_protect}">
						{/if}
		...
		<div class="btn-group pt-2" id="dropdownAdvancedFilter">
			...
			<ul class="dropdown-menu filtrePlanning" style="width:800px;overflow-x:scroll">
				<li>
					...
					{if ($filtreGroupeLieu|@count >0) or ($filtreGroupeRessource|@count >0)}<a href="process/planning?desactiverFiltreAvances=1" class="btn btn-danger btn-sm margin-left-10">{#formFiltreAvancesDesactiver#}</a>{/if}
				</li>
				<li class="divider"></li>
				<li>
						...
						<tr>
							<td class="planningDropdownFilter">
								...
								<label class="checkbox">
									<input type="checkbox" id="{$statust.status_id}" name="statutsTache[]" value="{$statust.status_id}" {if in_array($statust.status_id, $filtreStatutTache) || $filtreStatutTache|@count eq 0}checked="checked"{/if} />&nbsp;{$statust.nom|xss_protect}
								</label>
								{/foreach}
								</div>
							</td>
							<td class="planningDropdownFilter">
								...
								<label class="checkbox">
									<input type="checkbox" id="statut_projet_{$statusp.status_id}" name="statutsProjet[]" value="{$statusp.status_id}" {if in_array($statusp.status_id, $filtreStatutProjet) || $filtreStatutProjet|@count eq 0}checked="checked"{/if} />&nbsp;{$statusp.nom|xss_protect}
								</label>
								{/foreach}
								</div>
							</td>

							{* Filtres avanc�s emplacement *}
			...
			<div class="dropdown-menu">
				{if $baseLigne eq "projets"}
					{foreach from=$triPlanningPossibleProjet item=triTemp}
						...
						<a class="dropdown-item" href="process/planning?triPlanning={$triTemp|urlencode}">{if $triTemp eq $triPlanning}<i class="fa fa-check" aria-hidden="true"></i>&nbsp;&nbsp;{else}<i style="margin-left:19px;">&nbsp;</i>{/if}{$smarty.config.$chaineTmp}</a>
					{/foreach}
		...
		{* TOGGLE POUR AFFICHER BOUTONS SUPPLEMENTAIRES EN MOBILE *}
		<div class="btn-group pt-2" id="btnMoreActionsToggle">
			<a class="btn btn-default" href="javascript:void(0);" onclick="toggleMoreActions(true);" title="{#hide_show_table#}">
		...
		<div id="moreActions">
			...
			<div class="btn-group pt-2" id="dropdownExport">
				...
				<div class="dropdown-menu" style="">
					...
					<a class="dropdown-item" href="export_csv"><i class="fa fa-fw fa-file-text-o" aria-hidden="true"></i> {#CSVExport#|xss_protect}</a>
					<a class="dropdown-item" href="export_csv_raw"><i class="fa fa-fw fa-file-text-o" aria-hidden="true"></i> {#CSVExportRaw#|xss_protect}</a>
					<a class="dropdown-item" href="javascript:xajax_choixPDF();void(0);"><i class="fa fa-fw fa-file-pdf-o" aria-hidden="true"></i> {#PDFExport#|xss_protect}</a>
					<a class="dropdown-item" href="export_xls" target="_blank"><i class="fa fa-fw fa-file-excel-o" aria-hidden="true"></i> {#xlsExport#|xss_protect}</a>
					<a class="dropdown-item" href="export_gantt" target="_blank"><i class="fa fa-fw fa-file-pdf-o" aria-hidden="true"></i> {#ganttExport#|xss_protect}</a>
					<a class="dropdown-item" href="export_pdf_calendrier" target="_blank"><i class="fa fa-fw fa-calendar-o" aria-hidden="true"></i> {#calendarExport#|xss_protect}</a>
					<a class="dropdown-item" href="javascript:xajax_choixIcal();void(0);"><i class="fa fa-fw fa-envelope-o" aria-hidden="true"></i> {#icalExport#|xss_protect}</a>
				</div>
			</div>
			...
			<div class="btn-group pt-2" id="dropdownLarge">
				{if $dimensionCase eq "reduit"}
					<a class="btn btn-default" title="{#menuPlanningLarge#}" href="process/planning?dimensionCase=large"><i class="fa fa-search-plus fa-lg fa-fw" aria-hidden="true"></i></a>
				{else}
					<a class="btn btn-default" title="{#menuPlanningReduit#}" href="process/planning?dimensionCase=reduit"><i class="fa fa-search-minus fa-lg fa-fw" aria-hidden="true"></i></a>
				{/if}
				...
				<div class="dropdown-menu">						
					{if $fleches eq '1'}
						<a class="dropdown-item" href="{$BASE}/process/planning?fleches=0">
						<i class="fa fa-check" aria-hidden="true"></i>&nbsp;&nbsp;
					{else}
						<a class="dropdown-item" href="{$BASE}/process/planning?fleches=1"><i style="margin-left:19px;">&nbsp;</i>
					{/if}
					{#scrolls_fleches#}</a>
				</div>
			</div>

			{* DIV POUR FILTRES PERSO *}
			<div class="btn-group pt-2" id="dropdownFiltresPerso">
				<button class="btn dropdown-toggle btn-default" data-toggle="dropdown" onclick="javascript:multiselecthide();"><i class="fa fa-filter fa-lg fa-fw" aria-hidden="true"></i><span class="d-none d-xl-inline-block">&nbsp;&nbsp;{#formFiltresPerso#}</span>&nbsp;<span class="caret"></span></button>
				<div class="dropdown-menu">
					<a class="dropdown-item" href="javascript:xajax_filtre_perso_form('');void(0);"><i class="fa fa-plus fa-lg fa-fw" aria-hidden="true"></i> {#creer_filtre_perso#}</a>
					{foreach from=$filtres_perso item=filtre}
						<a class="dropdown-item" href="javascript:xajax_filtre_perso_form('{$filtre.filtre_perso_id}');void(0);"><i class="fa fa-caret-right fa-lg fa-fw" aria-hidden="true"></i> {$filtre.filtre_perso_nom|xss_protect}</a>
					{/foreach}
				</div>
			</div>
							...
							{if $filtreTexte != ""}
								<div class="btn-group">
									...
									<ul class="dropdown-menu">
										<li><a href="process/planning?desactiverFiltreTexte=1">{#formFiltreUserDesactiver#}</a></li>
									</ul>
								</div>
							{/if}
						</div>
						<div id="btnLessActionsToggle">
							<a class="btn btn-default" href="javascript:void(0);" onclick="toggleMoreActions(false);" title="{#hide_show_table#}">
...
{* DIV POUR CHOIX AFFICHAGE - d�plac� au bon endroit au chargement de la page *}
<div class="d-nonez" id="dropdownDisplayPlanning">
	...
	<div class="dropdown-menu">
		...
		{if $smarty.session.baseLigne eq 'heures' && $smarty.session.baseColonne eq 'users' }
			<a class="dropdown-item" href="process/planning?baseLigne=heures&baseColonne=users">
			<i class="fa fa-check" aria-hidden="true"></i>&nbsp;&nbsp;
		{else}
			<a class="dropdown-item" href="process/planning?baseLigne=heures&baseColonne=users">
			<i style="margin-left:19px;">&nbsp;</i>
		...
		*}
		...
		{if $smarty.session.baseColonne eq 'users' and $smarty.session.baseLigne eq 'heures'}
			<a class="dropdown-item disabled" href="process/planning?baseColonne=heures"><i style="margin-left:19px;">&nbsp;</i>&nbsp;&nbsp;{#planningHeures#}</a>
			<a class="dropdown-item disabled" href="process/planning?baseColonne=jours"><i style="margin-left:19px;">&nbsp;</i>&nbsp;&nbsp;{#planningJours#}</a>
		{else}
		...
		{if $smarty.session.masquerLigneVide eq 0 }
			<a class="dropdown-item" href="process/planning?baseLigne={$smarty.session.baseLigne}&baseColonne={$smarty.session.baseColonne}&masquerLigneVide=1">
			<i class="fa fa-check" aria-hidden="true"></i>&nbsp;&nbsp;
		{else}
			<a class="dropdown-item" href="process/planning?baseLigne={$smarty.session.baseLigne}&baseColonne={$smarty.session.baseColonne}&masquerLigneVide=0">
			<i style="margin-left:19px;">&nbsp;</i>
		{/if}
		{#planningAfficherLignesVides#}</a>
		
		{if $smarty.session.afficherLigneTotal eq 1}
			<a class="dropdown-item" href="process/planning?baseLigne={$smarty.session.baseLigne}&baseColonne={$smarty.session.baseColonne}&afficherLigneTotal=0">
			<i class="fa fa-check" aria-hidden="true"></i>&nbsp;&nbsp;
		{else}
			<a class="dropdown-item" href="process/planning?baseLigne={$smarty.session.baseLigne}&baseColonne={$smarty.session.baseColonne}&afficherLigneTotal=1">
			<i style="margin-left:19px;">&nbsp;</i>
		{/if}
		{#planningAfficherTotal#}</a>
		
		{if $smarty.session.afficherLigneTotalTaches eq 1}
			<a class="dropdown-item" href="process/planning?baseLigne={$smarty.session.baseLigne}&baseColonne={$smarty.session.baseColonne}&afficherLigneTotalTaches=0">
			<i class="fa fa-check" aria-hidden="true"></i>&nbsp;&nbsp;
		{else}
			<a class="dropdown-item" href="process/planning?baseLigne={$smarty.session.baseLigne}&baseColonne={$smarty.session.baseColonne}&afficherLigneTotalTaches=1">
			<i style="margin-left:19px;">&nbsp;</i>
		{/if}
		{#planningAfficherTotalTaches#}</a>

		{if $smarty.session.afficherTableauRecap eq 1}
			<a class="dropdown-item" href="process/planning?baseLigne={$smarty.session.baseLigne}&baseColonne={$smarty.session.baseColonne}&afficherTableauRecap=0">
			<i class="fa fa-check" aria-hidden="true"></i>&nbsp;&nbsp;
		{else}
			<a class="dropdown-item" href="process/planning?baseLigne={$smarty.session.baseLigne}&baseColonne={$smarty.session.baseColonne}&afficherTableauRecap=1">
			<i style="margin-left:19px;">&nbsp;</i>
		{/if}
