	...
	<div class="row">
		<div class="col-md-12">
			<div class="soplanning-box">
				<div class="btn-group">
					<a href="{$BASE}/options?tab=param-audit" class="btn btn-default" ><i class="fa fa-cogs fa-lg fa-fw" aria-hidden="true"></i>&nbsp;&nbsp;{#menuOptions#}</a>
				</div>
				<div class="btn-group">
					<a href="javascript:xajax_purgerAudit();void(0);"	onclick="javascript: return confirm('{#audit_purge_question#|xss_protect}')" class="btn btn-default" ><i class="fa fa-trash fa-lg fa-fw" aria-hidden="true"></i>&nbsp;&nbsp;{#audit_purge#}</a>
				</div>
			</div>
		</div>
	</div>
	{/if}
	<form action="audit" method="POST" id="filtreAudit">
		<div class="row">
		<div class="col-md-12">
			<div class="soplanning-box">
							...
							{else}
								...
								{foreach from=$users item=userCourant}
									{if $userCourant.user_groupe_id neq $groupeTemp}
										</optgroup><optgroup id="e{$userCourant.user_groupe_id}" label="{$userCourant.groupe_nom|xss_protect}">
									{/if}
							...
							{else}
								...
								{foreach from=$listeProjets item=projetCourant name=loopProjets}
									{if $projetCourant.groupe_id neq $groupeTemp}
										</optgroup><optgroup id="g{$projetCourant.groupe_id}" label="{$projetCourant.groupe_nom|xss_protect}">
									{/if}
						...
						{foreach name=audits item=audits from=$audit}
							<tr>
								...
								{if ($audits.type=="MT" or $audits.type=="DT" or $audits.type=="MP" or $audits.type=="DP" or $audits.type=="MG" or $audits.type=="DG" or $audits.type=="MU" or $audits.type=="DU" or $audits.type=="ME" or $audits.type=="DE" or $audits.type=="ML" or $audits.type=="DL" or $audits.type=="MR" or $audits.type=="DR" or $audits.type=="MS" or $audits.type=="DS") and $audits.nbmodifs > 0}
									<a href="javascript:xajax_modifAudit('{$audits.audit_id}');void(0);"><i class="fa fa-history fa-lg fa-fw" aria-hidden="true"></i></a>
								{/if}
								</td>
								<td>
									{$audits.date_modif}&nbsp;
								...
								<td class="wrap">
									{if $audits.type=="MT"}{#action_modif_tache#} {$audits.periode_id} : {$audits.informations|xss_protect}{/if}
									{if $audits.type=="AT"}{#action_ajout_tache#} {$audits.periode_id} : {$audits.informations|xss_protect}{/if}
									{if $audits.type=="DT"}{#action_suppression_tache#} {$audits.periode_id} : {$audits.informations|xss_protect}{/if}
									{if $audits.type=="AP"}{#action_ajout_projet#} "{$audits.informations|xss_protect}"{/if}									
									{if $audits.type=="MP"}{#action_modif_projet#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="DP"}{#action_suppression_projet#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="AG"}{#action_ajout_groupe#} "{$audits.informations|xss_protect}"{/if}									
									{if $audits.type=="MG"}{#action_modif_groupe#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="DG"}{#action_suppression_groupe#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="AU"}{#action_ajout_utilisateur#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="MU"}{#action_modif_utilisateur#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="DU"}{#action_suppression_utilisateur#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="AE"}{#action_ajout_equipe#} "{$audits.informations|xss_protect}"{/if}									
									{if $audits.type=="ME"}{#action_modif_equipe#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="DE"}{#action_suppression_equipe#} "{$audits.informations|xss_protect}"{/if}									
									{if $audits.type=="AL"}{#action_ajout_lieu#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="ML"}{#action_modif_lieu#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="DL"}{#action_suppression_lieu#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="AR"}{#action_ajout_ressource#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="MR"}{#action_modif_ressource#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="DR"}{#action_suppression_ressource#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="AS"}{#action_ajout_statut#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="MS"}{#action_modif_statut#} "{$audits.informations|xss_protect}"{/if}
									{if $audits.type=="DS"}{#action_suppression_statut#} "{$audits.informations|xss_protect}"{/if}
...
$("#filtreUserAudit").multiselect({
	...
	nameSuffix: 'user',
	desactivateUrl: 'audit?desactiverFiltreUserAudit=1',
...
$("#filtreGroupeProjetAudit").multiselect({
	...
	noUpdatePlaceholderText:true,
	nameSuffix: 'projet',
	desactivateUrl: 'audit?desactiverFiltreGroupeProjetAudit=1',
