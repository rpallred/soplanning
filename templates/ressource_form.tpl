...
<form method="POST" action="" target="_blank">
	...
	<div class="form-group row col-md-12 align-items-center">
		<label for="ressource_nom" class="col-md-4 col-form-label">{#winProjet_couleur#} :</label>
		<div class="col-md-3">
			<input name="couleur" id="couleur" maxlength="6" {if $smarty.session.isMobileOrTablet==1}type="color"{else}type="text"{/if} value="#{if $ressource.couleur eq ''}ffffff{else}{$ressource.couleur}{/if}" />
		</div>
	</div>
	<div class="form-group row col-md-12 align-items-center">
	...
	<div class="form-group row col-md-12 align-items-center">
		...
		<div class="col-md-5">
			<br />
			<input type="button" value="{#enregistrer#|escape:"html"}" class="btn btn-primary" onClick="xajax_submitFormRessource('{$ressource.ressource_id}', $('#ressource_id').val(), $('#ressource_nom').val(), $('#ressource_commentaire').val(), $('#ressource_exclusif').is(':checked'), $('#couleur').val())" />
		</div>
	</div>
</form>
<script>
