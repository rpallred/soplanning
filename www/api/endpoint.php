...
$server = Server::create('/')
	...
	->addGetRoute('users', function($id = '', $name = ''){
		checkAuth();
		$users = new GCollection('User');
		$sql = "SELECT *
				FROM planning_user 
				WHERE 0 = 0
				AND user_id <> 'publicspl' ";
		if ($id != '') {
			$sql .= "AND user_id LIKE " . val2sql('%' . $id . '%');
		}
		if ($name != '') {
	...
	///////////////// PROJECTS /////////////////////////////
	->addGetRoute('projects', function($id = '', $name = ''){
		checkAuth();
		...
		$sql = "SELECT *
				...
				WHERE 0 = 0 ";
		if ($id != '') {
			$sql .= "AND projet_id LIKE " . val2sql('%' . $id . '%');
		}
		if ($name != '') {
			$sql .= "AND nom LIKE " . val2sql('%' . $name . '%');
		}
		$sql .= "ORDER BY projet_id ASC";
	...
	->addGetRoute('statutes', function(){
		checkAuth();
		$statutes = new GCollection('Status');
		$statutes->db_load(array( ), array('priorite' => 'ASC'));
		$data = array();
		while($status = $statutes->fetch()){
