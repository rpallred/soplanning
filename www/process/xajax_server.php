...
function ajoutProjet($origine = null)
{
    ...
    // status par d?faut
    $status2 = new Status();
    $status2->db_loadSql("select status_id from planning_status where affichage in ('p','tp') and defaut='1' and affichage_liste=1 ORDER BY priorite limit 1");
...
function submitFormProjet($projet_id, $origine, $new_projet_id, $nom, $groupe_id, $statut, $livraison, $lien, $couleur, $createur_id, $iteration)
{
    ...
    if ($origine != '') {
        if ($origine == 'projets') {
            $objResponse->addRedirect('projets');
            return $objResponse;
        }
    }

    $objResponse->addRedirect('planning');
    return $objResponse;
}
...
function ajoutPeriode($dateDebut = '', $ligne_id = '', $periode_id = '', $heureDebut = '')
{
    ...
    if ($user->checkDroit('tasks_modify_own_project')) {
        $sql = "SELECT ppr.*, pg.nom AS nom_groupe
                ...
                WHERE  0 = 0
                AND (
                        (ppr.createur_id =  " . val2sql($user->user_id) . " AND planning_status.affichage_liste = '1')
                    )
        ...
        $sql = "SELECT DISTINCT ppr.*, pg.nom AS nom_groupe
                ...
                LEFT JOIN planning_status ON planning_status.status_id = ppr.statut
                WHERE (planning_status.affichage_liste = '1'
                        AND (ppe.periode_id IS NOT NULL OR ppr.createur_id = " . val2sql($user->user_id) . "))
                ORDER BY pg.nom, nom ASC
                ";
	} elseif ($user->checkDroit('tasks_view_specific_projects')) {
        $sql = "SELECT ppr.*, pg.nom AS nom_groupe
                FROM planning_projet AS ppr
                LEFT JOIN planning_groupe pg ON pg.groupe_id = ppr.groupe_id
                LEFT JOIN planning_status ON planning_status.status_id = ppr.statut
				INNER JOIN planning_right_on_projet AS rop ON rop.allowed_id = ppr.projet_id AND rop.owner_id = " . val2sql($user->user_id) . "
                WHERE  0 = 0
                AND planning_status.affichage_liste = '1'
                ORDER BY pg.nom, ppr.nom ASC
                ";
	} else {
        $sql = "SELECT ppr.*, pg.nom AS nom_groupe
    ...
    if ($user->checkDroit('tasks_modify_all') || $user->checkDroit('tasks_modify_own_project') || $user->checkDroit('tasks_modify_own_task') || $user->checkDroit('tasks_modify_team')) {
        ...
        if ($user->checkDroit('tasks_modify_own_task') || $user->checkDroit('tasks_view_only_own')) {
    ...
    // Liste des cr?neaux horaires exclusif
    ...
    if ($periode_id != '') {
        ...
        $sql = "SELECT distinct(user_id) from planning_periode where periode_id=" . val2sql($periode->periode_id) . " or link_id='" . $periode->link_id . "'";
		$listeTaskUserData = array();
    ...
    // hack pour textarea, pour ?viter l'interpretation de caract?res sp?ciaux
...
function modifPeriode($periode_id)
{
    ...
    if ($user->checkDroit('tasks_modify_own_project')) {
        ...
        $sql = "SELECT ppr.*, pg.nom AS nom_groupe
                ...
                WHERE  0 = 0
                AND (
                        planning_status.affichage_liste = '1'
                        OR ppr.projet_id = " . val2sql($projet->projet_id) . "
                    )
                ORDER BY pg.nom, ppr.nom ASC
                ";
    ...
    // Liste des users de la t?ches
    $listeTaskUsers = new GCollection('User');
    $sql = "SELECT distinct(user_id) from planning_periode where periode_id = " . val2sql($periode->periode_id) . " or link_id=" . val2sql($periode->link_id);
    $listeTaskUsers->db_loadSQL($sql);
    foreach ($listeTaskUsers->getSmartyData() as $u) {
        $listeTaskUserData[] = $u['user_id'];
    }
    ...
    if ($periode->estFilleOuParente()) {
        ...
        $smarty->assign('derniereOccurence', $periode->derniereOccurence());
    }
    ...
    // hack pour lien, pour ?viter l'interpretation de caract?res sp?ciaux
...
function moveCasePeriode($casePeriode, $jourCible, $copie_periode = false, $scope = 'seule')
{
    ...
    if ($scope == 'seule') {
        $sql = "SELECT distinct(periode_id),user_id from planning_periode where periode_id=" . val2sql($chaines1[1]);
    }else {
		$sql = "SELECT distinct(periode_id),user_id from planning_periode where periode_id=" . val2sql($chaines1[1]) . " or link_id = " . val2sql($data_periode['link_id']);
	}
    $listePeriode->db_loadSQL($sql);
	while ($p2 = $listePeriode->fetch()) {
    ...
    // v?rification si lors du drag'n'drop on change le user
    ...
    foreach ($listePeriodeModif as $p) {
        ...
        if ($copie_periode == 'true') {
			...
			// mise a jour des infos de la p?riode d?plac?e
			if ($projetCible->isSaved()) {
				$periode->projet_id = $projetCible->projet_id;
			} elseif($userCible->isSaved()) {
				// si c'est un d?placement de user, on g?n?re un nouveau link_id
				if ($type_move_user == true)
				{
					...
					if ($scope == "seule")
					{
						...
						// copie des fichiers joints vu que la t?che est d?li?e des autres
			...
			// reset de l'envoi si déjà envoyé la notif mais tache déplacée
			if ($periode->date_debut != $periodeBackup->date_debut || $periode->date_fin != $periodeBackup->date_fin || $periode->duree_details != $periodeBackup->duree_details) {
				$periode->notif_avant_date_envoyee = NULL;
			}
			// reset de l'envoi si déjà envoyé la notif mais tache déplacée
			if ($periode->date_debut != $periodeBackup->date_debut || $periode->date_fin != $periodeBackup->date_fin || $periode->duree_details != $periodeBackup->duree_details) {
				$periode->notif_apres_date_envoyee = NULL;
			}
		   ...
		   //modif ajout gestion des occurences
            if ($scope == 'toutes') {
				...
				// on dossocie d'une eventuelle soeur
	 ...
	 // Verification si le r?pertoire upload est encore n?cessaire 
	 $sql="SELECT periode_id FROM planning_periode where link_id = " . val2sql($old_link_id);
...
function modifUser($user_id = '', $copyFrom = '')
{
    global $lang, $default_palette;
    $objResponse = new xajaxResponse('ISO-8859-1');
    $smarty = new MySmarty();
	...
	if($copyFrom != ''){
		$userCopie = new User();
        $userCopie->db_load(array('user_id', '=', $copyFrom));
		$user_form->user_groupe_id = $userCopie->user_groupe_id;
		$user_form->droits = $userCopie->droits;
		$user_form->decoderDroits();
	}
    ...
    $smarty->assign('listProjetsRights', $user_form->getRightsOnProjets());
    ...
    // recuperation de la liste des projets pour filtre sur projets
    $projetsFiltre = new GCollection('Projet');
    $sql = "SELECT proj.*, pg.nom AS groupe_nom
            FROM planning_projet AS proj ";
    $sql .= " LEFT JOIN planning_groupe pg ON proj.groupe_id = pg.groupe_id ";
    $sql .= " ORDER BY groupe_nom, pg.nom";
...
function submitFormUser($user_id, $user_id_origine, $user_groupe_id, $nom, $email, $login, $password, $visible_planningOui, $couleur, $notificationsOui, $envoiMailPwd, $droits, $adresse, $telephone, $mobile, $metier, $commentaire, $login_actifOui, $specific_users_ids, $google_2fa_reset, $tarif_horaire_defaut, $specific_projets_ids, $langue)
{
    $objResponse = new xajaxResponse();
    $smarty = new MySmarty();
    $user = new User();
	...
	if($user_id_origine == ''){
		...
		$user_id_tmp = $user_id_origine;
		if (!$user_form->db_load(array('user_id', '=', $user_id_tmp))) {
			$objResponse->addAlert(addslashes($smarty->getConfigVars('ajax_droitsInsuffisants')));
			$objResponse->addScript('location.reload();');
			return $objResponse->getXML();
		}
    ...
    if ($user_id_origine == '') {
        ...
        if (trim($login) != '' && $userTest->db_load(array('login', '=', trim($login)))) {
 		...
 		if ($user_id != $user_id_origine) {
		   // si user existant on v?rifie que le nouveau user_id existe pas dï¿½jï¿½
			$userTest = new USer();
			if ($userTest->db_load(array('user_id', '=', $user_id))) {
				$objResponse->addAlert($smarty->getConfigVars('user_id_existant'));
				return $objResponse;
			...
			// modification de la clï¿½ (user_id) => update manuel
			$sql = 'UPDATE planning_user SET user_id = ' . val2sql($user_id) . ' WHERE user_id = ' . val2sql($user_id_origine);
			db_query($sql);
		}
	}
    ...
    if (trim($email) != '' && !VerifierAdresseMail(trim($email))) {
        $objResponse->addAlert($smarty->getConfigVars('user_emailInvalide'));
        return $objResponse;
    }

    if ($user->checkDroit('users_manage_team') && in_array('users_manage_all', $droits)) {
		$objResponse->addAlert(addslashes($smarty->getConfigVars('ajax_droitsInsuffisants')));
		$objResponse->addScript('location.reload();');
		return $objResponse->getXML();
	}
    ...
    $user_form->nom = $nom;
    $user_form->email = (trim($email) != '' ? trim($email) : null);
	...
	if ($langue == '') {
		// si pas de langue (on laisse vide par défaut), on force à la langue de l'utilisateur qui le crée
		// le but est qu'il voit par défaut un champ vide pour choisir sa langue si besoin
		$user_form->langue = $_SESSION['language'];
	} else {
		$user_form->langue = $langue;
	}
	if($tarif_horaire_defaut != $user_form->tarif_horaire_defaut){
		$changementTarifHoraire = true;
	}
    ...
    if ($password != '') {
		if(CONFIG_PASSWORD_COMPLEXITY == 1 && !verifierComplexiteMotDePasse($password)){
			$objResponse->addAlert($smarty->getConfigVars('erreur_complexite_motdepasse'));
			return $objResponse;
		}
    ...
    $user_form->couleur = ($couleur != '' && $couleur != '#' ? substr($couleur, 1, 6) : null);
	...
	if(isset($changementTarifHoraire)){
		$user_form->updateProjetsBudgets();
	}
    ...
    $user_form->updateRightsOnProjets($specific_projets_ids);
...
function supprimerUser($user_id)
{
    ...
    $_SESSION['message'] = 'changeOK';
    $objResponse->addRedirect('user_list');
    return $objResponse;
}
...
function submitFormProfil($user_id, $email, $password, $dateformat, $notificationsOui, $positionPlanning, $vueDefautPlanning, $vueDefautPersonne, $vueDefautMois, $vueDefautLarge, $masquerLigneVide, $afficherTableauRecap, $crsf)
{
	...
	if ($masquerLigneVide == '1') {
        $preferences['masquerLigneVide'] = '1';
    } else {
        $preferences['masquerLigneVide'] = '0';
    }

    if ($afficherTableauRecap == '1') {
        $preferences['afficherTableauRecap'] = '1';
    } else {
        $preferences['afficherTableauRecap'] = '0';
    }
    ...
    if (isset($_SESSION['preferences']['vuePlanning']) && ($_SESSION['preferences']['vuePlanning'] == "vueTaches") && (CONFIG_SOPLANNING_OPTION_TACHES == 1)) {
        $objResponse->addRedirect('taches');
    } else {
        $objResponse->addRedirect('planning');
    }
...
function submitFormPeriode($periode_id, $projet_id, $user_id, $date_debut, $conserver_duree, $date_fin, $nb_jours, $duree, $heure_debut, $heure_fin, $matin, $apresmidi, $repetition, $dateFinRepetitionJour, $dateFinRepetitionSemaine, $dateFinRepetitionMois, $nbRepetitionJour, $nbRepetitionSemaine, $nbRepetitionMois, $jourSemaine, $exceptionRepetition, $appliquerATous, $statut_tache, $lieu, $ressource, $livrable, $titre, $notes, $lien, $custom, $fichiers, $link_id, $notif_email, $pause, $updateoccurrences, $notif_standard_destinataires, $notif_avant_actif, $notif_avant_destinataires, $notif_avant_delai_nb, $notif_avant_delai_type, $notif_apres_actif, $notif_apres_destinataires, $notif_apres_delai_nb, $notif_apres_delai_type, $notif_apres_statuts_requis) {
    ...
    $periode->link_id = ($link_id != '' ? cleanStr($link_id) : uniqid(mt_rand()));
    $periode->date_debut = userdate2sqldate($date_debut);
    $periode->date_fin = userdate2sqldate($date_fin);
	
	// nettoyage des noms des fichiers comme dans process/upload.php
	if ($fichiers != '') {
		$tab = explode(';', $fichiers);
		$tabFinal = array();
		foreach ($tab as $ficTemp) {
			$infos = pathinfo(mb_convert_encoding($ficTemp, 'ISO-8859-1', 'UTF-8'));
			$filename = cleanStr($infos['filename'])  . '.' . $infos['extension'];
			$tabFinal[] = $filename;
		}
	    $periode->fichiers = implode(';', $tabFinal);
	} else {
    ...
    if ($conserver_duree === 'true') {
        ...
        while ($i <= $nbJours) {
            ...
            if (in_array($dateFin->format('N'), $DAYS_INCLUDED) && !array_key_exists($dateFin->format('Y-m-d'), $joursFeries)) {
                $i++;
            }
        }
	...
	if ($notif_email == 'true') {
		if (is_array($notif_standard_destinataires) && count($notif_standard_destinataires) > 0) {
			$tabUsers = array();
			foreach ($notif_standard_destinataires as $tmp_user_id) {
				$userCheck = new User();
				if ($userCheck->db_load(array('user_id', '=', $tmp_user_id))) {
					$tabUsers[] = $tmp_user_id;
				}
			}
			$periode->notif_standard_destinataires = implode(',', $tabUsers);
		} else {
			$periode->notif_standard_destinataires = NULL;
		}
	}

	if ($notif_avant_actif == 'true') {
		$periode->notif_avant_actif = 'oui';
		if (is_array($notif_avant_destinataires) && count($notif_avant_destinataires) > 0) {
			$tabUsers = array();
			foreach ($notif_avant_destinataires as $tmp_user_id) {
				$userCheck = new User();
				if ($userCheck->db_load(array('user_id', '=', $tmp_user_id))) {
					$tabUsers[] = $tmp_user_id;
				}
			}
			$periode->notif_avant_destinataires = implode(',', $tabUsers);
		} else {
			$periode->notif_avant_destinataires = NULL;
		}
		if ((int)$notif_avant_delai_nb <= 0) {
            $objResponse->addScript("document.getElementById('butSubmitPeriode').disabled=false;");
            $objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
            $objResponse->addAlert(addslashes($smarty->getConfigVars('notif_avant_delai_nb_erreur')));
            return $objResponse;
		}
		$periode->notif_avant_delai_nb = (int)$notif_avant_delai_nb;
		
		if (!in_array($notif_avant_delai_type, array('jour','heure','minute'))) {
            $objResponse->addScript("document.getElementById('butSubmitPeriode').disabled=false;");
            $objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
            $objResponse->addAlert(addslashes($smarty->getConfigVars('erreur')));
            return $objResponse;
		}
		$periode->notif_avant_delai_type = $notif_avant_delai_type;
	
		// reset de l'envoi si déjà envoyé la notif mais tache déplacée
		if (!isset($creationPeriode)) {
			if ($periode->date_debut != $periodeSave->date_debut || $periode->date_fin != $periodeSave->date_fin || $periode->duree_details != $periodeSave->duree_details) {
				$periode->notif_avant_date_envoyee = NULL;
			}
		}
		
	} else {
		$periode->notif_avant_actif = 'non';
		$periode->notif_avant_destinataires = NULL;
		$periode->notif_avant_delai_nb = NULL;
		$periode->notif_avant_delai_type = NULL;
		$periode->notif_avant_date_envoyee = NULL;
	}
	
	if ($notif_apres_actif == 'true') {
		$periode->notif_apres_actif = 'oui';
		if (is_array($notif_apres_destinataires) && count($notif_apres_destinataires) > 0) {
			$tabUsers = array();
			foreach ($notif_apres_destinataires as $tmp_user_id) {
				$userCheck = new User();
				if ($userCheck->db_load(array('user_id', '=', $tmp_user_id))) {
					$tabUsers[] = $tmp_user_id;
				}
			}
			$periode->notif_apres_destinataires = implode(',', $tabUsers);
		} else {
			$periode->notif_apres_destinataires = NULL;
		}
		if ((int)$notif_apres_delai_nb <= 0) {
            $objResponse->addScript("document.getElementById('butSubmitPeriode').disabled=false;");
            $objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
            $objResponse->addAlert(addslashes($smarty->getConfigVars('notif_apres_delai_nb_erreur')));
            return $objResponse;
		}
		$periode->notif_apres_delai_nb = (int)$notif_apres_delai_nb;
		
		if (!in_array($notif_apres_delai_type, array('jour','heure','minute'))) {
            $objResponse->addScript("document.getElementById('butSubmitPeriode').disabled=false;");
            $objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
            $objResponse->addAlert(addslashes($smarty->getConfigVars('erreur')));
            return $objResponse;
		}
		$periode->notif_apres_delai_type = $notif_apres_delai_type;

		if (is_array($notif_apres_statuts_requis) && count($notif_apres_statuts_requis) > 0) {
			$tabStatus = array();
			foreach ($notif_apres_statuts_requis as $tmp_status_id) {
				$statusCheck = new Status();
				if ($statusCheck->db_load(array('status_id', '=', $tmp_status_id))) {
					$tabStatus[] = $tmp_status_id;
				}
			}
			if (count($tabStatus) == 0) {
				$objResponse->addScript("document.getElementById('butSubmitPeriode').disabled=false;");
				$objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
				$objResponse->addAlert(addslashes($smarty->getConfigVars('notif_apres_statuts_requis_erreur')));
				return $objResponse;
			}
			$periode->notif_apres_statuts_requis = implode(',', $tabStatus);
		} else {
            $objResponse->addScript("document.getElementById('butSubmitPeriode').disabled=false;");
            $objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
            $objResponse->addAlert(addslashes($smarty->getConfigVars('notif_apres_statuts_requis_erreur')));
		...
		$periode->notif_apres_actif = 'non';
		$periode->notif_apres_destinataires = NULL;
		$periode->notif_apres_delai_nb = NULL;
		$periode->notif_apres_delai_type = NULL;
		$periode->notif_apres_date_envoyee = NULL;
	}
    ...
    // V?rification que le lieu est disponible
    if (!is_null($periode->lieu_id)) {
        if (!checkConflitLieu($lieu, $periode->$date_debut, $periode->$date_fin, $periode->duree_details, $user_id, $periode_id, $periode->link_id)) {
            $objResponse->addScript("document.getElementById('butSubmitPeriode').disabled=false;");
            $objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
    ...
    foreach ($user_id as $current_user_id) {
		...
		$personnesAssignees[] = $current_user_id;
	}
	...
	//on controle d'abord les droits
    foreach ($usersTraitement as $current_user_id => $todo) {
		...
		if($todo == 'update'){
			if(!$periodeTmp->db_load(array('user_id', '=', $current_user_id, 'link_id', '=', $periode->link_id, 'date_debut', '=', $periodeSave->date_debut))){
				echo 'Task not found - ' . $current_user_id . ' - '. $periode->link_id . ' - ' . $periodeSave->date_debut . ' - please contact support';die;
			}
		}
        ...
        if ($user->checkDroit('tasks_view_specific_projects')) {
			$r = new Right_on_projet();
			if(!$r->db_load(array('owner_id', '=', $user->user_id, 'allowed_id', '=', $projet->projet_id))){
				$objResponse->addScript("document.getElementById('butSubmitPeriode').disabled=false;");
				$objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
				$objResponse->addAlert(addslashes($smarty->getConfigVars('ajax_droitsInsuffisants')));
				return $objResponse;
    ...
    foreach ($usersTraitement as $current_user_id => $todo) {
		...
		// valeurs sauvee ici pour avoir les infos de la tache à jour
		if(!isset($new_data_for_audit)){
			$new_data_for_audit = $periodeTmp->getData();
		}

        if ($repetition != '' && $repetition != 'undefined') {
			// on supprime les rï¿½petitions existantes postï¿½rieures pour recreer les nouvelles 
			$sql = "DELETE FROM planning_periode WHERE parent_id = " . val2sql($periodeTmp->parent_id) . " AND date_debut > " . val2sql($periodeTmp->date_debut);
			db_query($sql);
	...
	// on fait la notification ici et non dans le db_save() sinon ca va s'appliquer à toutes les taches filles
	// on envoie que sur la derniere tache sauvée car de toute façon ça va parcourir l'ensemble des taches soeurs et notifier tout le monde
	if ($creationPeriode) {
		$periodeTmp->envoiNotification('creation', $repetition, $personnesAssignees);
	} else {
		$periodeTmp->envoiNotification('modification', $repetition, $personnesAssignees);
	}
    ...
    if (!isset($creationPeriode)) {
        if ($appliquerATous === 'true') {
            ...
            // Mise ? jour de toutes les occurrences
    ...
    if (CONFIG_SOPLANNING_OPTION_AUDIT == 1 && CONFIG_SOPLANNING_OPTION_AUDIT_TACHES == 1) {
		$infos = array();
		$infos['new_data'] = $new_data_for_audit;
        if (isset($periodeSave)) {
            ...
            $old_data = null;
            if (isset($new_data_for_audit['titre'])) {
                $infos['informations'] = $new_data_for_audit['titre'];
            } else {
                $infos['informations'] = "'" . $new_data_for_audit['date_debut'];
                if (!empty($old_data['date_fin'])) {
                    $infos['informations'] .= " => " . $new_data_for_audit['date_fin'] . "'";
                } else {
                    $infos['informations'] .= "'";
                }

                $infos['informations'] .= " (" . $new_data_for_audit['user_id'] . ")";
            }
            $action = "AT";
        }
...
function supprimerPeriode($periode_id, $fullscope = 'true', $notif_email = 'false')
{
    ...
    if (!$user->peutGererTache($periode)) {
        $objResponse->addAlert(addslashes($smarty->getConfigVars('ajax_droitsInsuffisants')));
        $objResponse->addScript('location.reload();');
        return $objResponse->getXML();
    }
    ...
    if (($nb_periodes_liees == 0) && (file_exists(UPLOAD_DIR . $periode->link_id))) {
        rrmdir(UPLOAD_DIR . $periode->link_id);
    }

    // Audit
    if (CONFIG_SOPLANNING_OPTION_AUDIT == 1 && CONFIG_SOPLANNING_OPTION_AUDIT_TACHES == 1) {
        $old_data = $periode->getData();
        $action = "DT";
        $infos['new_data'] = null;
        $infos['old_data'] = $old_data;
...
function submitFormFerie($date_ferie, $libelle, $couleur)
{
    ...
    $_SESSION['message'] = 'changeOK';
    $objResponse->addRedirect('feries');
    return $objResponse;
}
...
function icalGenererLien($anciennete, $ical_users = '', $ical_projets = '', $ical_projets_cb = array())
{
    $objResponse = new xajaxResponse();
    $smarty = new MySmarty();
    ...
    $lienIcal = $user->lienIcal($anciennete, $users, $projets);
    $objResponse->addAssign('inputLienIcal', 'value', $lienIcal);
	$objResponse->addAssign('downloadIcal', 'href', $lienIcal);
...
function modifLieu($lieu_id = null)
{
    ...
    $objResponse->addScript('jQuery("#myModal").modal()');
    $objResponse->addScript("jQuery('#couleur').spectrum({color: '#" . ($lieu->couleur != '' ? $lieu->couleur : 'ffffff') . "',showInput: true, allowEmpty:true, showPalette: true, showSelectionPalette: true, palette: [ $default_palette ], preferredFormat: 'hex',  chooseText: '" . $smarty->getConfigVars('colorpicker_valider') . "', cancelText: '" . $smarty->getConfigVars('colorpicker_annuler') . "', localStorageKey:'projet'});");

	return $objResponse->getXML();
}

function submitFormLieu($lieu_id, $new_lieu_id, $nom, $commentaire, $exclusif, $couleur)
{
    $objResponse = new xajaxResponse();
    $smarty = new MySmarty();
	...
	$lieu->couleur = ($couleur != '' ? $couleur : null);
...
function submitFormStatus($status_id, $new_status_id, $nom, $commentaire, $affichage, $barre, $gras, $italique, $souligne, $defaut, $affichage_liste, $pourcentage, $couleur, $priorite, $inclure_dans_calcul)
{
    $objResponse = new xajaxResponse();
    $smarty = new MySmarty();
    $user = new User();
	...
	if($inclure_dans_calcul != $status->inclure_dans_calcul){
		$updateProjets = true;
	}
	$status->inclure_dans_calcul = $inclure_dans_calcul;
	...
	if(isset($updateProjets)){
		$status->updateBudgetsProjet();
	}
	...
	if(isset($creation)){
		$_SESSION['filtreStatutTache'][] = $status->status_id;
		$_SESSION['filtreStatutProjet'][] = $status->status_id;
	}

    $_SESSION['message'] = 'changeOK';
    $objResponse->addRedirect('status');
    return $objResponse;
}
...
function modifRessource($ressource_id = null)
{
    ...
    $objResponse->addScript('jQuery("#myModal").modal()');
    $objResponse->addScript("jQuery('#couleur').spectrum({color: '#" . ($ressource->couleur != '' ? $ressource->couleur : 'ffffff') . "',showInput: true, allowEmpty:true, showPalette: true, showSelectionPalette: true, palette: [ $default_palette ], preferredFormat: 'hex',  chooseText: '" . $smarty->getConfigVars('colorpicker_valider') . "', cancelText: '" . $smarty->getConfigVars('colorpicker_annuler') . "', localStorageKey:'projet'});");

	return $objResponse->getXML();
}

function submitFormRessource($ressource_id, $new_ressource_id, $nom, $commentaire, $exclusif, $couleur)
{
    $objResponse = new xajaxResponse();
    $smarty = new MySmarty();
	...
	$ressource->couleur = ($couleur != '' ? $couleur : null);
   ...
   if ($exclusif == 'true') {$ressource->exclusif = 1;
    ...
    $_SESSION['message'] = 'changeOK';
    $objResponse->addRedirect('ressources');
    return $objResponse;
}
...
function usersBulkRightsSubmit($bulk_users_ids, $droits, $specific_users_ids, $specific_projets_ids)
{
    $objResponse = new xajaxResponse();
    $smarty = new MySmarty();
    ...
    foreach ($bulk_users_ids as $bulk_users_id) {
		...
		if($user->checkDroit('users_manage_team') && in_array('users_manage_all', $droits)){
			$objResponse->addAlert('Only for admin');
			return $objResponse;
		}
        ...
        $userTmp->updateRightsOnProjets($specific_projets_ids);
    }
...
function modifAudit($audit_id = null)
{
    ...
    foreach ($new_data as $cle => $val) {
        ...
        if (!empty($old_data[$cle])) {
            $valeurs[$cle]['old'] = mb_convert_encoding($old_data[$cle], 'ISO-8859-1', 'UTF-8');
        } else {
            $valeurs[$cle]['old'] = null;
        }

        $valeurs[$cle]['new'] = mb_convert_encoding($new_data[$cle], 'ISO-8859-1', 'UTF-8');
    }
...
function projet_copie_form($projet_id)
{
    global $lang;
    $objResponse = new xajaxResponse('ISO-8859-1');
    $smarty = new MySmarty();
	...
	$sql = "SELECT	*
			...
			LIMIT 1
	";
	$tacheDebut = new Periode();
	$tacheDebut->db_loadSQL($sql);
    $smarty->assign('tacheDebut', $tacheDebut->getSmartyData());
	...
	$sql = "SELECT	*
			...
			LIMIT 1
	";
	$tacheFin= new Periode();
	$tacheFin->db_loadSQL($sql);
    $smarty->assign('tacheFin', $tacheFin->getSmartyData());

    $objResponse->addScript('jQuery("#myModal .modal-header h5").html("' . addslashes($smarty->getConfigVars('projet_copie_infobulle')) . '")');
    $objResponse->addScript('jQuery("#myModal .modal-body").html("' . xajaxFormat($smarty->getHtml('projet_copie_form.tpl')) . '")');
    $objResponse->addScript('jQuery("#myModal").modal()');
...
function projet_copie_submit($projet_id, $projet_id_copie, $nom_copie, $radio_copie_a_partir, $date_debut_copie, $radio_copie_jusqua, $date_fin_copie, $date_demarrage)
{
    $objResponse = new xajaxResponse();
    $smarty = new MySmarty();
    $user = new User();
    ...
    if (trim($projet_id_copie) == '' || !preg_match('<^[A-Za-z0-9]+$>', $projet_id_copie)) {
        $objResponse->addScript("document.getElementById('butSubmitCopie').disabled=false;");
        $objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
        $objResponse->addAlert(addslashes($smarty->getConfigVars('projet_copie_projet_id_copie_erreur')));
        return $objResponse;
    }
    $projetTest = new Projet();
    $sql = 'SELECT * FROM planning_projet WHERE projet_id = ' . val2sql($projet_id_copie);
    if ($projetTest->db_loadSQL($sql)) {
        $objResponse->addScript("document.getElementById('butSubmitCopie').disabled=false;");
        $objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
        $objResponse->addAlert(addslashes($smarty->getConfigVars('projet_existant')));
    ...
    if (trim($nom_copie) == '') {
        $objResponse->addScript("document.getElementById('butSubmitCopie').disabled=false;");
        $objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
        $objResponse->addAlert(addslashes($smarty->getConfigVars('projet_copie_nom_copie_erreur')));
        return $objResponse;
    }
    ...
    if ($radio_copie_a_partir == 'radio_copie_a_partir_date' && ($date_debut_copie == '' || !controlDate($date_debut_copie))) {
        $objResponse->addScript("document.getElementById('butSubmitCopie').disabled=false;");
        $objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
    ...
    if ($radio_copie_jusqua == 'radio_copie_jusqua_date' && ($date_fin_copie == '' || !controlDate($date_fin_copie))) {
        $objResponse->addScript("document.getElementById('butSubmitCopie').disabled=false;");
        $objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
	...
	$data = $projet->getData();
	unset($data['saved']);
	$new_projet->setData($data);
	$new_projet->projet_id = $projet_id_copie;
	$new_projet->nom = $nom_copie;
	$new_projet->createur_id = $user->user_id;
	$new_projet->db_save();
    ...
    $sql = "    SELECT *
                FROM planning_periode
                WHERE projet_id = " . val2sql($projet->projet_id);
	if($radio_copie_a_partir == 'radio_copie_a_partir_date'){
		$sql .= "
                AND
                    (
	...
	if($radio_copie_jusqua == 'radio_copie_jusqua_date'){
		$sql .= "
                AND
                    (
	...
	// on calcule le delta ï¿½ partir de la premiere tache trouvï¿½e
	$tmp = $taches->fetch();
    ...
    while ($tache = $taches->fetch()) {
		...
		$new_tache->link_id = uniqid(mt_rand());
		$new_tache->projet_id = $new_projet->projet_id;
...
function selection_multi_tache_form($tabDivTaches)
{
	...
	foreach ($tabDivTaches as $div){
		$data2 = explode('_', $div);
		$tache = new Periode();
		if($tache->db_load(array('periode_id', '=', $data2[1]))){
			if(!array_key_exists($data2[1], $tabTaches)){
				$tabTaches[$data2[1]] = $tache->getSmartyData();
			}
		}
	}
    $smarty->assign('taches', $tabTaches);
    $smarty->assign('chaineTaches', implode(',', array_keys($tabTaches)));

    $objResponse->addScript('jQuery("#myModal").modal("hide")');
    $objResponse->addScript('jQuery("#myModal .modal-header h5").html("' . addslashes($smarty->getConfigVars('ajax_titreCreationProjet')) . '")');
    $objResponse->addScript('jQuery("#myModal .modal-body").html("' . xajaxFormat($smarty->getHtml('action_multi_taches_form.tpl')) . '")');
    $objResponse->addScript('jQuery("#myModal").modal()');
...
function tutoriel_masquer()
{
	...
	$user->set_valeur_tutoriel('masquer', 1);
...
function projet_dupliquer_tache_submit($projet_id, $date_debut_copie, $date_fin_copie, $date_demarrage)
{
    ...
    // s'il n'y a pas de tache pendant cette pï¿½riode
    if ($tmp != false) {
        ...
        while ($tache = $taches->fetch()) {
            ...
            // si l'id n'est pas sauvegardï¿½, si l'identifiant n'est pas sauvegardï¿½,
            // en crï¿½er un nouveau et l'attribuer ï¿½ celui qui vient d'ï¿½tre gï¿½nï¿½rï¿½
            ...
            // en gardant le mï¿½me links_id d'utilisateur dans la mï¿½me tï¿½che
...
function projet_couts_submit($projet_id, $budget_montant, $budget_temps, $users, $tarifs)
{
	...
	// si de nouveaux users ajoutï¿½s on recharge juste la fenetre, sinon on la ferme
	if(count($users) > 0){
		$objResponse->addScript('jQuery("#myModal").modal("hide")');
...
function filtre_perso_form($filtre_perso_id = null)
{
    $objResponse = new xajaxResponse('ISO-8859-1');
    $smarty = new MySmarty();
    ...
    $filtre = new Filtre_perso();
    if ($filtre_perso_id != '') {
        $filtre->db_load(array('filtre_perso_id', '=', $filtre_perso_id));

		if($filtre->user_id != $user->user_id){
			$objResponse->addAlert(addslashes($smarty->getConfigVars('ajax_droitsInsuffisants')));
			return $objResponse;
		}
	}

    $smarty->assign('filtre_perso', $filtre->getSmartyData());

    $objResponse->addScript('jQuery("#myModal .modal-header h5").html("' . addslashes($smarty->getConfigVars('formFiltresPerso')) . '")');
    $objResponse->addScript('jQuery("#myModal .modal-body").html("' . xajaxFormat($smarty->getHtml('filtre_perso_form.tpl')) . '")');
    $objResponse->addScript('jQuery("#myModal").modal()');

	return $objResponse->getXML();
}

function filtre_perso_save($filtre_perso_id, $filtre_perso_nom)
{
    $objResponse = new xajaxResponse();
    $smarty = new MySmarty();
	...
	if ($filtre_perso_nom == "") {
        $objResponse->addAlert(addslashes($smarty->getConfigVars('user_nomManquant')));
        return $objResponse;
    }
	
	$check = new Filtre_perso();
	if($filtre_perso_id != ''){
		if($check->db_load(array('filtre_perso_id', '<>', $filtre->filtre_perso_id, 'filtre_perso_nom', '=', $filtre_perso_nom))){
			$objResponse->addAlert(addslashes($smarty->getConfigVars('filtre_perso_nom_existant')));
			return $objResponse;
		}
	} else{
		if($check->db_load(array('filtre_perso_nom', '=', $filtre_perso_nom))){
			$objResponse->addAlert(addslashes($smarty->getConfigVars('filtre_perso_nom_existant')));
			return $objResponse;
		}
	}
    ...
    $filtre->filtre_perso_nom = $filtre_perso_nom;
	$filtre->user_id = $user->user_id;

	$contenu = array();
	$contenu['users'] = $_SESSION['filtreUser'];
	$contenu['projets'] = $_SESSION['filtreGroupeProjet'];
	$contenu['statuts_taches'] = $_SESSION['filtreStatutTache'];
	$contenu['statuts_projets'] = $_SESSION['filtreStatutProjet'];
	$contenu['lieux'] = $_SESSION['filtreGroupeLieu'];
	$contenu['ressources'] = $_SESSION['filtreGroupeRessource'];

	$filtre->filtre_perso_contenu = json_encode($contenu);
...
function filtre_perso_appliquer($filtre_perso_id)
{
    $objResponse = new xajaxResponse();
    $smarty = new MySmarty();
	...
	$filtre = new Filtre_perso();
	if(!$filtre->db_load(array('filtre_perso_id', '=', $filtre_perso_id))){
		$objResponse->addAlert(addslashes($smarty->getConfigVars('ajax_droitsInsuffisants')));
	}
	...
	$contenu = json_decode($filtre->filtre_perso_contenu, true);
	$_SESSION['filtreUser'] = $contenu['users'];
	$_SESSION['filtreGroupeProjet'] = $contenu['projets'];
	$_SESSION['filtreStatutTache'] = $contenu['statuts_taches'];
	$_SESSION['filtreGroupeLieu'] = $contenu['lieux'];
	$_SESSION['filtreGroupeRessource'] = $contenu['ressources'];
	unset($_COOKIE["filtreStatutTache"]);
	setcookie('filtreStatutTache', '', time() - 3600, '/');
	$_SESSION['filtreStatutProjet'] = $contenu['statuts_projets'];
...
function filtre_perso_delete($filtre_perso_id)
{
    $objResponse = new xajaxResponse();
    $smarty = new MySmarty();
