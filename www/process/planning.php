...
if(isset($_POST['filtreGroupeProjet'])) {
	...
	die;
	*/
	$tab = validerTabProjets($_POST, 'projet');
	$_SESSION['filtreGroupeProjet'] = $tab;
	setcookie('filtreGroupeProjet', json_encode($tab), time()+60*60*24*500, '/');
}
...
if(isset($_POST['filtreUser'])) {
	$tab = validerTabUsers($_POST, 'user');
    setcookie('filtreUser', implode(",", $tab), time() + 60*60*24*365, '/');
	$_SESSION['filtreUser'] = $tab;
}

if(isset($_GET['filtreSurUser'])) {
	$tab = validerTabUsers(array($_GET['filtreSurUser']));
    setcookie('filtreUser', implode(",", $tab), time() + 60*60*24*365, '/');
	$_SESSION['filtreUser'] = $tab;
}
