...
$fichier_import_seul=$filename;
$type="save";
$save_dir = SAVE_DIR."$filename".".tmp." . $_SESSION['randomizeUpload'];
...
function restore_parametres()
{
	...
	while ( ($data = fgetcsv($handle, 1000, ";") ) !== FALSE ) {
		...
		$config->cle = $data[0];
		$config->valeur = ($data[1] != '' ? xss_protect($data[1]) : null);
		$config->commentaire = ($data[1] != '' ? xss_protect($data[1]) : null);
...
function restore_projets()
{
	...
	while ( ($data = fgetcsv($handle, 1000, ";") ) !== FALSE ) {
		...
		$projet->lien = ($data[7] != '' ? xss_protect($data[7]) : null);
...
function restore_groupes_projets()
{
	...
	while ( ($data = fgetcsv($handle, 1000, ";") ) !== FALSE ) {
		...
		$groupe->nom = ($data[1] != '' ? xss_protect(substr($data[1],0, $groupe->getFieldSize('nom'))) : null);
...
function restore_taches()
{
	...
	while ( ($data = fgetcsv($handle, 1000, ";") ) !== FALSE ) {
        ...
        // Vérification de l'existence de l'enregistrement
		if (trim($data[0]) != '' && $periode->db_load(array('periode_id', '=', $data[0]))) {
			// pas d'écrasement, on ignore l'enregistrement
			if ($type_restauration==1)
			{
		...
		$periode->titre = ($data[9] != '' ? xss_protect(substr($data[9],0, $periode->getFieldSize('titre'))) : null);
		$periode->notes = ($data[10] != '' ? xss_protect(substr($data[10],0, $periode->getFieldSize('notes'))) : null);
		$periode->lien = ($data[11] != '' ? xss_protect($data[11]) : null);
		...
		$periode->custom = ($data[21] != '' ? xss_protect($data[21]) : null);
		$periode->pause = ($data[22] != '' ? $data[22] : null);
		if ($type<>"ignore")
		{
...
function restore_users()
{
	...
	while ( ($data = fgetcsv($handle, 2000, ";") ) !== FALSE ) {
		...
		$user_form->nom = xss_protect(substr($data[2],0, $user_form->getFieldSize('nom')));
		...
		$user_form->droits = xss_protect(($data[8] != '' ? $data[8] : null));
		if($data[9] != ''){
		...
		$user_form->adresse = ($data[11] != '' ? xss_protect($data[11]) : null);
		$user_form->telephone = ($data[12] != '' ? xss_protect($data[12]) : null);
		$user_form->mobile = ($data[13] != '' ? xss_protect($data[13]) : null);
		$user_form->metier = ($data[14] != '' ? xss_protect($data[14]) : null);
		$user_form->commentaire = ($data[15] != '' ? xss_protect($data[15]) : null);
...
function restore_lieux()
{
	...
	while ( ($data = fgetcsv($handle, 1000, ";") ) !== FALSE ) {
		...
		// on ajoute ou met à jour les données
		$lieu->lieu_id = $data[0];
		$lieu->nom = ($data[1] != '' ? xss_protect(substr($data[1],0, $lieu->getFieldSize('nom'))) : null);
		$lieu->commentaire = ($data[2] != '' ? xss_protect(substr($data[2],0, $lieu->getFieldSize('commentaire'))) : null);
...
function restore_ressources()
{
	...
	while ( ($data = fgetcsv($handle, 1000, ";") ) !== FALSE ) {
		...
		$ressource->nom = ($data[1] != '' ?  xss_protect(substr($data[1],0, $ressource->getFieldSize('nom'))) : null);
		$ressource->commentaire = ($data[2] != '' ? xss_protect(substr($data[2],0, $ressource->getFieldSize('commentaire'))) : null);
...
function restore_status()
{
	...
	while ( ($data = fgetcsv($handle, 1000, ";") ) !== FALSE ) {
		...
		$status->nom = ($data[1] != '' ? xss_protect(substr($data[1],0, $status->getFieldSize('nom'))) : null);
		$status->commentaire = ($data[2] != '' ? xss_protect($data[2]) : null);
...
function restore_feries()
{
	...
	while ( ($data = fgetcsv($handle, 1000, ";") ) !== FALSE ) {
		...
		$feries->libelle = ($data[1] != '' ? xss_protect($data[1]) : null);
		$feries->couleur = ($data[2] != '' ? xss_protect($data[2]) : null);
		if ($type<>"ignore")
		{
			if ($feries->db_save())
			{
