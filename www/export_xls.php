...
while ($tmpDate <= $dateFin) {
	...
	*/
	$headerNomJours .= '<th class="' . $sClass . '" style="width:25px;">' . strtoupper(substr($smarty->getConfigVars('day_' . $tmpDate->format('N')), 0, 1)) . '</th>' . CRLF;
...
if($_SESSION['baseLigne'] == 'projets') {
	...
	if($user->checkDroit('tasks_view_specific_projects')) {
		$sql .= " INNER JOIN planning_right_on_projet AS rop ON rop.allowed_id = planning_projet.projet_id AND rop.owner_id = " . val2sql($user->user_id);
	}
	if($user->checkDroit('tasks_view_own_projects')) {
