...
function remplirDateFinPeriode() {
	if (document.getElementById('date_fin').value == '') {
		...
		if (dateDebut != '') {
			...
			jQuery("#date_fin").datepicker('update', dateDebut);
		}
	}
}
...
// Intervalle minimum entre deux traitements allowDrop (en ms)
const ALLOW_DROP_INTERVAL = 50;
let lastAllowDropTime = 0;
...
// get input value for the given prefix
function getInputs(formName, inputPrefix) {
	var resultat = new Array();
	var inputList = document.forms[formName].elements;
	var compteur = 0;
	for(compteur;compteur!=inputList.length;compteur++){
		var str = inputList[compteur].id;
		var tab = str.split("_");
		if (tab[0] == inputPrefix) {
			resultat[tab[1]] = inputList[compteur].value;
		}
	}
	return resultat;
}
...
function allowDrop(ev) {
	...
	// Throttling : on ne traite pas plus souvent que ALLOW_DROP_INTERVAL
	var now = Date.now();
	if (now - lastAllowDropTime < ALLOW_DROP_INTERVAL) {
		return;
	}
	lastAllowDropTime = now;
...
function leaveDropZone(ev) {
	...
	//console.log('leaveDropZone ' + ev.target.id);
}
...
function fileUpload() {
    ...
    var filename=$('#fichier')[0].files[0]['name'].sansAccent();
...
function toggleMoreActions(show) {
    var container = document.getElementById('moreActions');
    var opener = document.getElementById('btnMoreActionsToggle');
    if (!container) { return; }
    if (show === true) {
    ...
    if (opener) {
        if (container.classList.contains('show')) { opener.classList.add('hidden'); }
