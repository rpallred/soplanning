...
$soplanningUrlBrute = str_replace(array('https://', 'http://', '/'), array('', '', ''), CONFIG_SOPLANNING_URL);

$v = new vcalendar( array( 'unique_id' => 'SOPlanning@' . $soplanningUrlBrute, "TZID" => date_default_timezone_get()));
$v->setProperty( 'X-WR-CALNAME', 'SOPlanning' . (CONFIG_SOPLANNING_TITLE != 'SOPlanning' ? ' - ' . CONFIG_SOPLANNING_TITLE : ''));
...
while($lineTmp = $lines->fetch()) {
	...
	while ($periode = $periodes->fetch()) {
		...
		$e->setProperty('summary' , mb_convert_encoding($nomTache,'UTF-8' , 'ISO-8859-1'));
		$description = $periode->notes;
		$lienTache = $periode->getTaskLink();
		if($lienTache != ''){
			$description .=  " " . $smarty->getConfigVars('tab_lien') . ' : ' . $lienTache;
		}

		$e->setProperty('description', mb_convert_encoding($description,'UTF-8' , 'ISO-8859-1'));
		$e->setProperty('uid', $periode->periode_id . '@' . $soplanningUrlBrute);
