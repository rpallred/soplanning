...
if ($base_ligne=="users") {
	$linkswitch="process/planning?baseLigne=projets&baseColonne=$base_colonne";
} else {
	$linkswitch="process/planning?baseLigne=users";
}
// Autres variables
$droitAjoutPeriode = false;
$_SESSION['lastURL'] = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
// Conversion de la dur�e maximale du jour en quanti�me
...
// Date de d�but d'affichage du planning
...
if ($_SESSION['baseColonne']=="users" and ($base_ligne=="heures")) {
	...
	$smarty->assign('dateDebutTexte', $smarty->getConfigVars('day_' . $dateDebut->format('N')) . ' ' . $dateDebut->format(CONFIG_DATE_LONG));
	$smarty->assign('dateDebutTexte1', $smarty->getConfigVars('day_' . $dateDebut->format('N')));
...
$smarty->assign('dateFinTexte', $smarty->getConfigVars('day_' . $dateFin->format('N')) . ' ' . $dateFin->format(CONFIG_DATE_LONG));
$smarty->assign('dateFinTexte1', $smarty->getConfigVars('day_' . $dateFin->format('N')));
...
// P�riode pr�c�dente et suivante
...
// Date de livraison
// si param livraison existe, veut dire qu'on vient des projets et qu'on affiche la semaine demand�e
...
if(isset($_COOKIE['filtreGroupeProjet'])) {
	$_SESSION['filtreGroupeProjet'] = validerTabProjets(json_decode(html_entity_decode($_COOKIE['filtreGroupeProjet'])));
...
if(isset($_COOKIE['filtreGroupeLieu'])) {
	$_SESSION['filtreGroupeLieu'] = validerTabLieux(json_decode(html_entity_decode($_COOKIE['filtreGroupeLieu'])));
...
if(isset($_COOKIE['filtreGroupeRessource'])) {
	$_SESSION['filtreGroupeRessource'] = validerTabRessources(json_decode(html_entity_decode($_COOKIE['filtreGroupeRessource'])));
...
// Liste des projets couvrant la p�riode, pour le filtre de projets
...
// calcul des heures de d�but et de fin en fonction des heures coch�es dans les params
...
$filtres_perso = new GCollection('Filtre_perso');
$filtres_perso->db_load(array('user_id', '=', $user->user_id), array('filtre_perso_nom' => 'ASC'));
$smarty->assign('filtres_perso', $filtres_perso->getSmartyData());
