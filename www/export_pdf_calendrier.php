...
while ($tmpDate <= $dateFin) {
	...
	while (true) {
		...
		$html .= '>' . strtoupper(substr($smarty->getConfigVars('day_' . $tmpDate->format('N')), 0, 1)) . '</td>' . CRLF;
		$html .= '<td class="calCell"';
		if (in_array($tmpDate->format('N'), $DAYS_INCLUDED) && !array_key_exists($tmpDate->format('Y-m-d'), $joursFeries)) {
			$html .= ' class="calOpenDay"';
		} else {
		...
		if (isset($joursOccupes[$tmpDate->format('Y-m-d')]) && in_array($tmpDate->format('N'), $DAYS_INCLUDED) && !array_key_exists($tmpDate->format('Y-m-d'), $joursFeries)) {
