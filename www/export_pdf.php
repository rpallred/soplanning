...
// Dimensions des formats PDF en mm
$formats = array(
	'A0' => array('portrait' => array('largeur' => 841, 'hauteur' => 1189), 'paysage' => array('largeur' => 1189, 'hauteur' => 841)),
	'A1' => array('portrait' => array('largeur' => 594, 'hauteur' => 841), 'paysage' => array('largeur' => 841, 'hauteur' => 594)),
	'A2' => array('portrait' => array('largeur' => 420, 'hauteur' => 594), 'paysage' => array('largeur' => 594, 'hauteur' => 420)),
	'A3' => array('portrait' => array('largeur' => 297, 'hauteur' => 420), 'paysage' => array('largeur' => 420, 'hauteur' => 297)),
	'A4' => array('portrait' => array('largeur' => 210, 'hauteur' => 297), 'paysage' => array('largeur' => 297, 'hauteur' => 210))
);

// Marges HTML2PDF par défaut (en mm) - ces marges sont appliquées automatiquement par la librairie
$marge_gauche_pdf = 15;  // Marge gauche par défaut d'HTML2PDF
$marge_droite_pdf = 15;  // Marge droite par défaut d'HTML2PDF
$marge_tableau = 5;      // Marge supplémentaire pour le tableau
$largeur_colonne_nom = 52; // Largeur fixe de la colonne des noms (200px * 0.26mm)

// Espacement entre les cellules (borders, padding, etc.)
$espacement_cellules = 2; // mm par cellule pour les bordures et espacement

$format = isset($formats[$pdf_format]) ? $formats[$pdf_format] : $formats['A4'];
$dimensions = $format[$pdf_orientation];

// Largeur disponible pour les colonnes de dates (en tenant compte des marges PDF)
$largeur_disponible = $dimensions['largeur'] - $marge_gauche_pdf - $marge_droite_pdf - $marge_tableau - $largeur_colonne_nom;

// Convertir la largeur de cellule de pixels en mm (approximation: 1 pixel ≈ 0.26 mm)
$largeur_cellule_mm = CONFIG_PLANNING_COL_WIDTH * 0.26;

// Calculer le nombre de colonnes en tenant compte de l'espacement
// Formule: largeur_disponible = nb_colonnes * largeur_cellule + (nb_colonnes - 1) * espacement
// Donc: nb_colonnes = (largeur_disponible + espacement) / (largeur_cellule + espacement)
$colonnes_par_page = floor(($largeur_disponible + $espacement_cellules) / ($largeur_cellule_mm + $espacement_cellules));

// Minimum 3 colonnes par page pour éviter trop de pages
$colonnes_par_page = max(3, $colonnes_par_page);

//$_GET['debug_pagination'] = 1;
// Debug temporaire pour vérifier le calcul
if(isset($_GET['debug_pagination'])) {
	$format = isset($formats[$pdf_format]) ? $formats[$pdf_format] : $formats['A4'];
	$dimensions = $format[$pdf_orientation];
	$largeur_disponible = $dimensions['largeur'] - 15 - 15 - 5 - 52;
	$largeur_cellule_mm = CONFIG_PLANNING_COL_WIDTH * 0.26;
	$espacement_cellules = 2;
	$largeur_totale_utilisee = $colonnes_par_page * $largeur_cellule_mm + ($colonnes_par_page - 1) * $espacement_cellules;
	
	echo "Format: " . $pdf_format . "<br>";
	echo "Orientation: " . $pdf_orientation . "<br>";
	echo "Largeur page (mm): " . $dimensions['largeur'] . "<br>";
	echo "Marges PDF (gauche+droite): 30mm<br>";
	echo "Marge tableau: 5mm<br>";
	echo "Largeur colonne nom: 52mm<br>";
	echo "Largeur disponible: " . $largeur_disponible . "mm<br>";
	echo "Largeur cellule (pixels): " . CONFIG_PLANNING_COL_WIDTH . "<br>";
	echo "Largeur cellule (mm): " . $largeur_cellule_mm . "<br>";
	echo "Espacement cellules: " . $espacement_cellules . "mm<br>";
	echo "Colonnes par page calculées: " . $colonnes_par_page . "<br>";
	echo "Largeur totale utilisée: " . $largeur_totale_utilisee . "mm<br>";
	echo "Espace restant: " . ($largeur_disponible - $largeur_totale_utilisee) . "mm<br>";
	die;
}

// Fonction pour générer les en-têtes d'une page
function genererEnTetesPage($dateDebut, $dateFin, $dateDebutPage, $dateFinPage, $smarty, $joursFeries, $DAYS_DISPLAYED, $DAYS_GREYEDOUT) {
	$headerMois = '' . CRLF;
	$headerSemaines = '' . CRLF;
	$headerNomJours = '' . CRLF;
	...
	while ($tmpDate <= $dateFinPage) {
		...
		$headerNomJours .= '<th width="' . CONFIG_PLANNING_COL_WIDTH . '" class="planning_head_day ' . $sClass . '">' . strtoupper(substr($smarty->getConfigVars('day_' . $tmpDate->format('N')), 0, 1)) . '</th>' . CRLF;
	...
	return array(
		'headerMois' => $headerMois,
		'headerSemaines' => $headerSemaines,
		'headerNomJours' => $headerNomJours,
		'headerNumeroJours' => $headerNumeroJours
	);
}

// Calculer les dates de début et fin pour chaque page
$pages = array();
$dateCourante = clone $dateDebut;
$colonne_courante = 0;

while ($dateCourante <= $dateFin) {
	$dateDebutPage = clone $dateCourante;
	$dateFinPage = clone $dateCourante;
	$colonnes_page = 0;
	
	// Compter les colonnes pour cette page
	while ($dateCourante <= $dateFin && $colonnes_page < $colonnes_par_page) {
		if (in_array($dateCourante->format('N'), $DAYS_DISPLAYED)) {
			$colonnes_page++;
		}
		$dateFinPage = clone $dateCourante;
	...
	$pages[] = array(
		'debut' => clone $dateDebutPage,
		'fin' => clone $dateFinPage
	);
}

// GÉNÉRATION DU CONTENU AVEC PAGINATION
$html = '';

// Pour chaque page
foreach ($pages as $indexPage => $page) {
	$dateDebutPage = $page['debut'];
	$dateFinPage = $page['fin'];
	
	// Générer les en-têtes pour cette page
	$enTetes = genererEnTetesPage($dateDebut, $dateFin, $dateDebutPage, $dateFinPage, $smarty, $joursFeries, $DAYS_DISPLAYED, $DAYS_GREYEDOUT);
	
	$html .= '<page orientation="' . ($pdf_orientation == 'paysage' ? 'L' : 'P') . '">' . CRLF;
	$html .= '<style>' . file_get_contents('assets/css/themes/'.CONFIG_SOPLANNING_THEME) . file_get_contents('assets/css/export_pdf.css') . '</style>' . CRLF;
	
	$html .= '<table class="planningContent">' . CRLF;
	$html .= '<tr>' . CRLF;
	$html .= '<th id="tdUser_0" rowspan="4" class="planning_switch" width="200"></th>' .CRLF;
	$html .= $enTetes['headerMois'] . CRLF;
	$html .= '</tr>' . CRLF;
	$html .= '<tr>' . CRLF;
	$html .= $enTetes['headerSemaines'] . CRLF;
	$html .= '</tr>' . CRLF;
	$html .= '<tr>' . CRLF;
	$html .= $enTetes['headerNomJours'] . CRLF;
	$html .= '</tr>' . CRLF;
	$html .= '<tr>' . CRLF;
	$html .= $enTetes['headerNumeroJours'] . CRLF;
	$html .= '</tr>' . CRLF;
...
while($ligneTmp = $lines->fetch()) {
	...
	if(strpos($_SESSION['triPlanning'], 'groupe_nom') !== FALSE || strpos($_SESSION['triPlanning'], 'team_nom') !== FALSE) {
		if($_SESSION['baseLigne'] == 'projets') {
			if($ligneTmp->groupe_nom !== $groupeCourant) {
				$html .= '<tr>' . CRLF;
				$html .= '<th class="planning_team_div" id="tdUser_' . $idGroupeCourant . '" width="200">&nbsp;' . ($ligneTmp->groupe_nom != '' ? xss_protect($ligneTmp->groupe_nom) : $smarty->getConfigVars('planning_pasDeGroupe')) . '&nbsp;' . CRLF;
				$html .= '</th>' . CRLF;
				$tmpDate = clone $dateDebutPage;
				while ($tmpDate <= $dateFinPage) {
					if (in_array($tmpDate->format('N'), $DAYS_DISPLAYED)) {
			...
			if($ligneTmp->team_nom !== $groupeCourant) {
				$html .= '<tr>' . CRLF;
				$html .= '<th class="planning_team_div" id="tdUser_' . $idGroupeCourant . '" width="200">&nbsp;' . ($ligneTmp->team_nom != '' ? xss_protect($ligneTmp->team_nom) : $smarty->getConfigVars('planning_pasDeTeam')) . '&nbsp;' . CRLF;
				$html .= '</th>' . CRLF;
				$tmpDate = clone $dateDebutPage;
				while ($tmpDate <= $dateFinPage) {
					if (in_array($tmpDate->format('N'), $DAYS_DISPLAYED)) {
	...
	$html .= '<tr>' . CRLF;
	$html .= '<th class="divpeoplePDF" id="tdUser_' . ($nbLine-1) . '" width="200" ' . ((!is_null($ligneTmp->couleur) && $ligneTmp->couleur != 'FFFFFF') ? ' style="background-color:#'.$ligneTmp->couleur. ';color:' . buttonFontColor('#' . $ligneTmp->couleur) . '"' : '') . '>&nbsp;' . $ligneTmp->nom . '&nbsp;</th>' . CRLF;
	$tmpDate = clone $dateDebutPage;
	// on boucle sur la dur?e de l'affichage de cette page
	while ($tmpDate <= $dateFinPage) {
		$styleTD = '';
...
// Le HTML est maintenant généré avec la pagination dans la boucle foreach ci-dessus

if(isset($_GET['cb_inclure_recap'])) {
	$html .= '<page pageset="old" orientation="' . ($pdf_orientation == 'paysage' ? 'L' : 'P') . '"><style>' . file_get_contents('assets/css/export_pdf.css') . '</style>';
...
//	$html2pdf = new HTML2PDF($orientation, $pdf_format, 'fr', true, 'iso-8859-1');
	...
	$html2pdf = new HTML2PDF(($pdf_orientation == 'paysage' ? 'L' : 'P'), $pdf_format, 'fr', true);
